...
class SelectiveTestRunner:
    ...
    def run_quick_tests(self) -> Dict:
        ...
        return self.run_tests(
            markers=["quick"], exclude_markers=["slow", "load"], timeout=60
        )
    ...
    def run_regression_tests(self) -> Dict:
        ...
        return self.run_tests(
            markers=["regression"], exclude_markers=["load"], timeout=300
        )
    ...
    def run_critical_tests(self) -> Dict:
        ...
        return self.run_tests(
            markers=["critical"], exclude_markers=["slow", "load"], timeout=180
        )

    def run_persona_tests(self) -> Dict:
        """Run persona-based user scenario tests."""
        print("🎭 Running persona tests...")
        return self.run_tests(markers=["persona"], timeout=600)

    def run_security_tests(self) -> Dict:
        """Run security baseline tests."""
        print("🔒 Running security tests...")
        return self.run_tests(markers=["security"], timeout=240)

    def run_ml_tests(self) -> Dict:
        """Run machine learning component tests."""
        print("🧠 Running ML tests...")
        return self.run_tests(markers=["ml"], timeout=300)

    def run_integration_tests(self) -> Dict:
        """Run integration tests."""
        print("🔗 Running integration tests...")
        return self.run_tests(
            markers=["integration"], exclude_markers=["load"], timeout=480
        )

    def run_performance_tests(self) -> Dict:
        """Run performance and load tests."""
        print("📊 Running performance tests...")
        return self.run_tests(markers=["performance", "load"], timeout=600)
...
def main():
    ...
    parser.add_argument(
        "--custom-markers", nargs="+", help="Custom markers to run (e.g., health cli)"
    )
    parser.add_argument(
        "--exclude", nargs="+", help="Markers to exclude (e.g., slow load)"
    )
    parser.add_argument(
    ...
    if args.list_markers:
        ...
        print(
            "  python scripts/run_selective_tests.py --custom-markers regression --exclude slow"
        )
        return 0
    ...
    if args.custom_markers:
        ...
        result = runner.run_tests(
            markers=args.custom_markers, exclude_markers=args.exclude
        )
