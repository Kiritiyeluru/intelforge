...
class JobQueue:
    ...
    def add_job(self, job: ScrapingJob):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                job.id, job.name, job.job_type, job.priority.value,
                job.schedule_expression, json.dumps(job.config),
                json.dumps(job.retry_policy), job.created_at, job.next_run,
                job.last_run, job.status.value, job.retry_count,
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor.fetchall():
                ...
                job = ScrapingJob(
                    ...
                    metadata=json.loads(job_data['metadata']) if job_data['metadata'] else None
                )
                jobs.append(job)
    ...
    def update_job_status(self, job_id: str, status: JobStatus, 
                         next_run: str = None, retry_count: int = None):
...
class EnterpriseScheduler:
    ...
    def execute_job(self, job: ScrapingJob) -> bool:
        ...
        # Log start event
        self.logger.log_event(ScrapingEvent(
            timestamp=datetime.now().isoformat(),
            event_type="start",
            scraper_type=job.job_type,
            url=job.config.get('urls', ['scheduled_job'])[0] if job.config.get('urls') else 'scheduled_job',
    ...
    def handle_job_failure(self, job: ScrapingJob, error_message: str):
        ...
        if retry_policy.should_retry(job.retry_count):
            ...
            self.log.info(f"Job {job.id} will retry in {delay:.1f}s (attempt {job.retry_count + 1})")
        else:
            # Mark as failed
            self.job_queue.update_job_status(job.id, JobStatus.FAILED)
            self.log.error(f"Job {job.id} failed permanently after {job.retry_count} retries")
    
    def execute_financial_batch(self, job: ScrapingJob) -> bool:
        """Execute financial batch scraping job."""
        config = job.config
        profile = self.profile_manager.get_profile(config.get('profile', 'financial_fast'))
        
        # Build command
        cmd = [
            sys.executable, "scripts/batch_stealth_scraper.py",
            "--workers", str(profile.get('workers', 3)),
            "--retries", str(profile.get('retries', 2)),
            "--delay-min", str(profile.get('delay_range', [2, 5])[0]),
            "--delay-max", str(profile.get('delay_range', [2, 5])[1])
        ]
        ...
        try:
            result = subprocess.run(cmd, cwd=project_root, capture_output=True, 
                                  text=True, timeout=profile.get('timeout', 300))
...
def main():
    ...
    parser.add_argument('--create-defaults', action='store_true', help='Create default scheduled jobs')
    
    # Job creation parameters
    parser.add_argument('--name', type=str, help='Job name')
    parser.add_argument('--type', type=str, choices=['financial_batch', 'academic_research', 'single_url'], help='Job type')
    parser.add_argument('--priority', type=str, choices=['low', 'medium', 'high', 'critical'], default='medium', help='Job priority')
    parser.add_argument('--schedule', type=str, default='every_1h', help='Schedule expression (e.g., every_30m, every_6h)')
    parser.add_argument('--urls', nargs='+', help='URLs to scrape')
    parser.add_argument('--profile', type=str, default='financial_fast', help='Scraping profile')
    ...
    if args.start:
        ...
        jobs_created.append(scheduler.create_job(
        ...
        jobs_created.append(scheduler.create_job(
            ...
            config={
                'queries': ['algorithmic trading', 'quantitative finance', 'financial risk management'],
                'max_results': 10
            }
        ))
        ...
        if jobs:
            ...
            for job in jobs:
                print(f"  - {job.id}: {job.name} ({job.priority.name}) - {job.status.value}")
        else:
            print("No ready jobs found")
    
    else:
        print("Use --start to run scheduler, --create-job to add jobs, or --create-defaults")
