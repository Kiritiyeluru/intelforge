...
def save_papers(papers: List[Dict], query: str, database: str, config: Dict) -> int:
    ...
    for paper in papers:
        ...
        filename = f"{title_slug}_{paper_id}_{datetime.now().strftime('%Y%m%d_%H%M')}.md"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Academic Research Script - Multi-Database Search")
    parser.add_argument('--query', type=str, required=True, help="Search query for academic papers")
    parser.add_argument('--database', type=str, default='arxiv', 
                       choices=['arxiv', 'pubmed'],
                       help="Academic database to search")
    parser.add_argument('--limit', type=int, default=20, help="Maximum number of papers to retrieve")
    ...
    print(f"Academic Research Script - Multi-Database Search")
    ...
    try:
        ...
        if papers:
            ...
            print(f"- Tool: paperscraper (381 stars, 5 databases)")
