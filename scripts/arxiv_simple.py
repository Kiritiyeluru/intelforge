...
def save_papers(papers: List[Dict], query: str, config: Dict) -> int:
    ...
    for paper in papers:
        ...
        filename = f"{title_slug}_{arxiv_id}_{datetime.now().strftime('%Y%m%d_%H%M')}.md"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ArXiv Research Script - Direct API Usage")
    parser.add_argument('--query', type=str, required=True, help="Search query for ArXiv papers")
    ...
    try:
        ...
        if papers:
            ...
            print(f"- Source: ArXiv (official API)")
            print(f"- Tool: arxiv.py (1.3k stars)")
            ...
            if papers:
                ...
                print(f"- Categories: {', '.join(first_paper.get('categories', [])[:3])}")
        else:
