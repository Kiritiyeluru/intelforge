        ...
        try:
            ...
            try:
                ...
                if not response or response.status >= 400:
                    self.logger.warning(
                        f"HTTP {response.status if response else 'No response'} for {url}"
                    )
                    return None
...
def main():
    ...
    parser.add_argument(
        "--urls-file", type=str, help="File containing URLs to scrape (one per line)"
    )
    parser.add_argument(
        "--headless", action="store_true", default=True, help="Run in headless mode"
    )
    parser.add_argument(
        "--show-browser", action="store_true", help="Show browser (non-headless mode)"
    )
    parser.add_argument(
