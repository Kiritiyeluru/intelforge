...
class WebSpider(scrapy.Spider):
    ...
    site_configs = {
        ...
        "quantstart.com": {
            "search_urls": ["https://www.quantstart.com/articles/"],
    ...
    def parse_search_page(self, response):
        ...
        for article_url in list(article_links)[:max_articles]:
            yield scrapy.Request(
                url=article_url, callback=self.parse_article, meta={"site": site}
            )
    ...
    def parse_article(self, response):
        ...
        try:
            ...
            title = (
                ...
                else self._extract_title_fallback(response)
            )
            author = (
                metadata.author if metadata and metadata.author else "Unknown Author"
            )
            ...
            item["site"] = response.meta.get(
                "site", self._get_site_from_url(response.url)
            )
