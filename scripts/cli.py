...
try:
    from scripts.utils.graceful_shutdown import (operation_tracking,
                                                 register_cleanup,
                                                 setup_cli_shutdown_handling)
    ...
    register_cleanup(
        lambda: logging.info("IntelForge CLI shutdown complete"), "cli_shutdown"
    )
...
try:
    from scripts.canary_validation_system_v2 import \
        main as canary_validator_main
    from scripts.semantic_crawler import main as semantic_crawler_main
    from scripts.validation.fail_fast_validator import (FailFastValidator,
                                                        cli_fail_fast_check)
...
def run_fail_fast_check(command: str) -> bool:
    ...
    try:
        if not cli_fail_fast_check(command):
            ...
            for failure in critical_failures:
                ...
                if failure.fix_suggestion:
                    console.print(
                        f"    💡 Fix: {failure.fix_suggestion}", style="yellow"
                    )
        ...
        typer.echo(
            f"⚠️ Validation check failed: {e}", err=True, color=typer.colors.YELLOW
        )
...
def crawl(
    ...
    threshold: float = typer.Option(
        0.75, "--threshold", "-t", help="Similarity threshold for content relevance"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="Preview mode - analyze content but don't save"
    ),
    regenerate_reference: bool = typer.Option(
        False, "--regenerate-reference", help="Regenerate reference embeddings"
    ),
    skip_validation: bool = typer.Option(
        False, "--skip-validation", help="Skip fail-fast validation checks"
    ),
    # Phase 4 Production CLI Flags
    limit_domains: Optional[str] = typer.Option(
        None,
        "--limit-domains",
        help="Comma-separated list of allowed domains to prevent crawl explosions",
    ),
    save_raw: bool = typer.Option(
        False, "--save-raw", help="Save raw HTML content for debugging before parsing"
    ),
    proxy_rotate: bool = typer.Option(
        False, "--proxy-rotate", help="Enable proxy rotation for stealth crawling"
    ),
    max_retries: int = typer.Option(
        3, "--max-retries", help="Maximum number of retries for failed requests"
    ),
    respect_robots: bool = typer.Option(
        True,
        "--respect-robots/--ignore-robots",
        help="Respect robots.txt files (default: enabled)",
    ),
):
    """
    ...
    if not skip_validation and not run_fail_fast_check("crawl"):
        typer.echo(
            "❌ Pre-flight validation failed. Use --skip-validation to override.",
            err=True,
        )
        raise typer.Exit(1)
...
def docs(
    ...
    doc_type: Optional[str] = typer.Option(
        None, "--type", "-t", help="Document type (STS, IMP, ARC, etc.)"
    ),
    priority: Optional[str] = typer.Option(
        None, "--priority", "-p", help="Priority level (A, B, C, D)"
    ),
    ...
    if action == "create":
        ...
        if script_path.exists():
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    console.print(
                        f"❌ Document creation failed: {result.stderr}", style="red"
                    )
                    raise typer.Exit(1)
            except Exception as e:
        ...
        if script_path.exists():
            ...
            try:
                ...
                if result.returncode == 0:
                    typer.echo(
                        "✅ Validation completed successfully", color=typer.colors.GREEN
                    )
                ...
                typer.echo(
                    f"❌ Error running validation script: {e}",
                    err=True,
                    color=typer.colors.RED,
                )
                raise typer.Exit(1)
        ...
        typer.echo(
            "📂 Document organization not yet implemented", color=typer.colors.YELLOW
        )
        typer.echo("Use 'intelforge docs validate' to check naming conventions")

    else:
        typer.echo(
            f"Error: Unknown action '{action}'. Use: create, validate, or organize",
            err=True,
        )
        raise typer.Exit(1)
...
def status(
    json_output: bool = typer.Option(
        False, "--json", help="Output JSON format for CI/automation"
    ),
    drift: bool = typer.Option(
        False, "--drift", help="Include semantic drift reporting"
    ),
    detailed: bool = typer.Option(
        False, "--detailed", help="Include detailed health checks"
    ),
    skip_validation: bool = typer.Option(
        False, "--skip-validation", help="Skip health validation checks"
    ),
):
    """
    ...
    for name, path in storage_dirs.items():
        ...
        if path.exists():
            try:
                ...
                status_data["storage"][name]["file_count"] = len(
                    [f for f in files if f.is_file()]
                )
                status_data["storage"][name]["size_bytes"] = sum(
    ...
    for name, path in config_files.items():
        status_data["config_files"][name] = {"exists": path.exists(), "path": str(path)}
    ...
    try:
        ...
        status_data["system_info"]["disk_free_gb"] = psutil.disk_usage(
            str(base_dir)
        ).free / (1024**3)
    ...
    if "health_checks" in status_data and status_data["health_checks"].get(
        "overall_status"
    ) in ["critical_failure", "high_failure"]:
...
def _display_rich_status(status_data: dict, detailed: bool = False):
    ...
    for name, storage in status_data["storage"].items():
        if storage["exists"]:
            ...
            storage_table.add_row(name.capitalize(), "❌ Not found", "-", "-")
    ...
    if status_data.get("system_info"):
        ...
        if "memory_available_gb" in sys_info:
            sys_table.add_row(
                "Available Memory", f"{sys_info['memory_available_gb']:.1f}GB"
            )
    ...
    if (
        "health_checks" in status_data
        and "error" not in status_data["health_checks"]
        and "overall_status" in status_data["health_checks"]
    ):
        ...
        health_table.add_row(
            "Status", f"[{health_color}]{health['overall_status'].upper()}[/]"
        )
        ...
        if detailed and "detailed_results" in health:
            ...
            for check in health["detailed_results"]:
                ...
                detail_table.add_row(
                    check["name"], check["level"].upper(), icon, check["message"]
                )
...
def snapshot(
    ...
    snapshot_path: Optional[str] = typer.Option(
        None, "--path", help="Snapshot path (auto-generated if not provided)"
    ),
    collection_name: str = typer.Option(
        "semantic_capture", "--collection", help="Collection name to snapshot"
    ),
):
    """
    ...
    try:
        ...
        if action == "create":
            ...
            console.print("✅ Snapshot created successfully:", style="green")
            ...
            if not snapshot_path:
                console.print(
                    "❌ Snapshot path required for restore action", style="red"
                )
                raise typer.Exit(1)

            console.print(
                "📥 Restoring vector storage from snapshot...", style="bold cyan"
            )
            ...
            console.print(
                f"❌ Unknown action '{action}'. Use: create, restore, or list",
                style="red",
            )
            raise typer.Exit(1)
...
def migrate(
    ...
    if from_storage.lower() == "qdrant" and to_storage.lower() == "chromadb":
        try:
            from scripts.vector_storage_migration import \
                migrate_qdrant_to_chroma

            console.print(
                "📦 Starting migration from Qdrant to ChromaDB...", style="bold blue"
            )
        ...
        console.print(
            f"❌ Migration from '{from_storage}' to '{to_storage}' not supported",
            style="red",
        )
...
def freshness(
    ...
    domain: Optional[str] = typer.Option(
        None, "--domain", help="Filter by specific domain"
    ),
    threshold_hours: int = typer.Option(
        24, "--threshold", help="Stale threshold in hours"
    ),
):
    """
    ...
    try:
        ...
        if action == "report":
            ...
            if domain:
                ...
                if metrics:
                    console.print(
                        f"📈 Freshness Report for {domain}:", style="bold blue"
                    )
                    console.print(f"  Total URLs: {metrics.total_urls}")
                    console.print(
                        f"  Fresh: {metrics.fresh_count} | Stale: {metrics.stale_count} | Expired: {metrics.expired_count}"
                    )
            ...
            domains = tracker.db.execute(
                "SELECT DISTINCT domain FROM crawl_records"
            ).fetchall()
            ...
            for domain_row in domains:
                ...
                if metrics:
                    updated_count += 1
                    console.print(
                        f"  ✅ Updated {domain_name}: {metrics.total_urls} URLs, {metrics.avg_age_hours:.1f}h avg age"
                    )

            console.print(
                f"✅ Updated metrics for {updated_count} domains", style="green"
            )
            ...
            console.print(
                f"✅ Cleaned up {deleted_count} old records (older than {cleanup_days} days)",
                style="green",
            )

        elif action == "stale":
            console.print(
                f"⏰ Finding stale URLs (older than {threshold_hours} hours)...",
                style="bold cyan",
            )
            ...
            if stale_urls:
                ...
                for url in stale_urls[:20]:  # Limit to first 20
                    table.add_row(
                        ...
                        url["status"],
                        (
                            url["error_message"][:30] + "..."
                            if url["error_message"] and len(url["error_message"]) > 30
                            else url["error_message"] or ""
                        ),
                    )
                ...
                if len(stale_urls) > 20:
                    console.print(
                        f"... and {len(stale_urls) - 20} more stale URLs", style="dim"
                    )
            else:
            ...
            console.print(
                f"❌ Unknown action '{action}'. Use: report, update, cleanup, or stale",
                style="red",
            )
            raise typer.Exit(1)
...
def pii_scan(
    content: Optional[str] = typer.Option(
        None, "--content", help="Content to scan for PII"
    ),
    file_path: Optional[Path] = typer.Option(
        None, "--file", help="File to scan for PII"
    ),
    output: Optional[Path] = typer.Option(
        None, "--output", help="Output file for scan report"
    ),
    risk_threshold: float = typer.Option(
        50.0, "--threshold", help="Risk threshold for safety check"
    ),
    use_presidio: bool = typer.Option(
        True, "--presidio/--basic", help="Use Presidio or basic regex detection"
    ),
):
    """
    ...
    try:
        ...
        console.print(
            f"{status_icon} Status: {'PII DETECTED' if scan_result['has_pii'] else 'CLEAN'}",
            style=f"bold {status_color}",
        )
        console.print(
            f"📈 Risk Score: {scan_result['risk_score']:.1f}/100", style=status_color
        )
        ...
        if scan_result["entities"]:
            ...
            for entity in scan_result["entities"][:10]:  # Show first 10
                console.print(
                    f"  • {entity['type']}: [dim]{entity['text']}[/dim] (confidence: {entity['confidence']:.2f})"
                )

            if len(scan_result["entities"]) > 10:
                console.print(
                    f"  ... and {len(scan_result['entities']) - 10} more entities"
                )
...
def version_callback(
    ...
    """Handle version flag and show version information"""
    if version:
        console.print(
            Panel.fit(
                f"[bold cyan]IntelForge[/bold cyan] v{VERSION}\n"
                f"[dim]Build: {BUILD_HASH}[/dim]\n"
                f"[dim]Phase: {PHASE}[/dim]\n"
                f"[dim]Status: Production Ready[/dim]",
...
def version_info():
    ...
    try:
        ...
        component_status = {}
        try:
            from scripts.semantic_crawler import ENHANCED_MODULES_AVAILABLE

            component_status["Enhanced Modules"] = (
                "✅ Available" if ENHANCED_MODULES_AVAILABLE else "⚠️  Basic Mode"
            )
        except:
            component_status["Enhanced Modules"] = "❌ Not Available"

        try:

            component_status["ChromaDB"] = "✅ Available"
        except:
            component_status["ChromaDB"] = "❌ Not Available"

        try:
            from scripts.utils.vector_security_manager import \
                VectorSecurityManager

            component_status["Security Manager"] = "✅ Available"
        except:
            component_status["Security Manager"] = "❌ Not Available"

        # Create version table
        table = Table(
            title="IntelForge Version Information",
            show_header=True,
        ...
        try:
            ...
            health_status = security_manager.get_security_health_check()

            console.print("\n🛡️  Security Health Status:", style="bold cyan")
            for component, status in health_status.items():
                if isinstance(status, dict):
                    status_text = (
                        "✅ Healthy"
                        if status.get("status") == "healthy"
                        else f"⚠️ {status.get('status', 'unknown')}"
                    )
                    console.print(
                        f"  • {component.replace('_', ' ').title()}: {status_text}"
                    )
        except Exception as e:
            console.print(f"\n⚠️ Security health check unavailable: {e}", style="yellow")

    except Exception as e:
        console.print(f"❌ Version information error: {e}", style="red")
        raise typer.Exit(1)


@app.command("budget-check")
def budget_check(
    warn_if_over: float = typer.Option(
        95.0, "--warn-if-over", help="Warn if budget exceeds percentage (default: 95%)"
    ),
    report: bool = typer.Option(False, "--report", help="Show detailed budget report"),
    add_time: Optional[str] = typer.Option(
        None, "--add-time", help="Add time entry: 'phase,task,hours,description'"
    ),
    ...
    try:
        ...
        if add_time:
            try:
                ...
                if len(parts) != 4:
                    typer.echo(
                        "❌ Invalid --add-time format. Use: 'phase,task,hours,description'",
                        err=True,
                    )
                    raise typer.Exit(1)
                ...
                tracker.add_time_entry(
                    phase.strip(), task.strip(), hours, description.strip()
                )
                typer.echo(
                    f"✅ Added {hours}h to {phase.strip()}/{task.strip()}",
        ...
        if not budget_ok:
            typer.echo(
                f"⚠️  Budget exceeds {warn_if_over}% threshold",
            ...
            if not report:
                ...
                typer.echo(
                    f"Current utilization: {summary['utilization_percent']:.1f}%"
                )
                typer.echo(
            ...
            if not (report or add_time or markdown):
                ...
                _, summary = tracker.get_overall_budget_status()
                typer.echo(
                    f"✅ Budget OK: {summary['utilization_percent']:.1f}% used",
                    color=typer.colors.GREEN,
                )
                typer.echo(
                    f"Spent: {summary['total_spent']:.1f}h / Allocated: {summary['total_allocated']:.1f}h"
...
def sync(
    input_file: Optional[Path] = typer.Option(
        None, "--input", "-i", help="URL file to crawl"
    ),
    threshold: float = typer.Option(
        0.75, "--threshold", "-t", help="Similarity threshold for content relevance"
    ),
    skip_crawl: bool = typer.Option(
        False, "--skip-crawl", help="Skip crawling, only run embeddings/snapshot"
    ),
    skip_embeddings: bool = typer.Option(
        False, "--skip-embeddings", help="Skip embedding generation"
    ),
    skip_snapshot: bool = typer.Option(
        False, "--skip-snapshot", help="Skip vector snapshot creation"
    ),
    skip_archive: bool = typer.Option(
        False, "--skip-archive", help="Skip archiving old data"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="Preview mode - analyze but don't save"
    ),
):
    """
    ...
    if not skip_crawl:
        if not input_file:
            console.print(
                "❌ Input file required for crawling. Use --input or --skip-crawl",
                style="red",
            )
            raise typer.Exit(1)
        ...
        try:
            if MODULES_AVAILABLE:
                ...
                vector_storage = ChromaVectorStorage(
                    "./chroma_storage", "semantic_capture"
                )
                ...
                console.print(
                    f"✅ Phase 3 complete: Snapshot created at {snapshot_metadata['snapshot_path']}",
                    style="green",
                )
            else:
                console.print(
                    "⚠️  Phase 3: ChromaDB modules not available", style="yellow"
                )
        except Exception as e:
...
def health(
    ...
    strict: bool = typer.Option(
        False, "--strict", help="Exit with non-zero code on any failures"
    ),
    detailed: bool = typer.Option(
        False, "--detailed", help="Include detailed health checks"
    ),
    ...
    try:
        if MODULES_AVAILABLE:
            ...
            health_data["checks"]["system_health"] = {
                "status": (
                    "healthy" if summary["overall_status"] == "healthy" else "degraded"
                ),
                "details": {
                    "total_checks": summary["total_checks"],
                    "results": summary["results"],
    ...
    try:
        ...
        if overall_metrics.get("domain_breakdown"):
            total_age = sum(
                domain["avg_age_hours"]
                for domain in overall_metrics["domain_breakdown"]
            )
...
def _display_rich_health(health_data: dict, detailed: bool = False):
    ...
    for check_name, check_data in health_data["checks"].items():
        ...
        if check_name == "system_health" and "total_checks" in check_data["details"]:
            ...
            check_name == "drift_status" and "drift_percentage" in check_data["details"]
        ):
            details = f"{check_data['details']['drift_percentage']}% drift"
        elif (
            check_name == "freshness_status"
            and "avg_age_hours" in check_data["details"]
        ):
            details = f"{check_data['details']['avg_age_hours']}h avg age"
        elif (
            check_name == "crawl_success_rate"
            and "success_rate" in check_data["details"]
        ):
