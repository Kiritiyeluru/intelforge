...
class BatchStealthScraper:
    ...
    def __init__(self, max_workers: int = 5, delay_range: tuple = (2, 5), 
                 retry_attempts: int = 3, use_browser: bool = False):
        """
        ...
        try:
            ...
            if self.use_browser:
                ...
                result = subprocess.run([
                    sys.executable, "scripts/stealth_scraper.py",
                    "--url", url
            ...
            if "Content Length:" in result.stdout:
                try:
                    ...
                    content_length = int(line.split(':')[1].strip().replace(',', '').split()[0])
                except:
                    pass
    ...
    def scrape_batch(self, urls: List[str], progress_callback: Optional[callable] = None) -> Dict[str, Any]:
        """
        ...
        print(f"⚙️  Configuration: {self.max_workers} workers, {'browser' if self.use_browser else 'HTTP'} method")
        print("-" * 60)
        
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Submit all jobs
            future_to_url = {executor.submit(self.scrape_with_retry, url): url for url in urls}
            ...
            for future in as_completed(future_to_url):
                ...
                print(f"{status} [{completed:3d}/{len(urls):3d}] {progress:5.1f}% | {url:<50} | {result['execution_time']:5.2f}s")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Batch Stealth Scraper - High-Performance Concurrent Web Scraping")
    ...
    parser.add_argument('--delay-min', type=int, default=2, help="Minimum delay between requests (default: 2s)")
    parser.add_argument('--delay-max', type=int, default=5, help="Maximum delay between requests (default: 5s)")
    parser.add_argument('--retries', type=int, default=3, help="Number of retry attempts (default: 3)")
    parser.add_argument('--browser', action='store_true', help="Use browser-based scraping (slower but more capable)")
    ...
    print(f"🎯 Batch Stealth Scraper - High-Performance Mode")
    print(f"📋 URLs to process: {len(unique_urls)} (duplicates removed: {len(urls) - len(unique_urls)})")
    print(f"⚙️  Workers: {args.workers} | Retries: {args.retries} | Method: {'Browser' if args.browser else 'HTTP'}")
