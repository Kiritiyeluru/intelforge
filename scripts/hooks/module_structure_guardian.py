...
def check_pattern_compliance(file_path, content_info, patterns):
    ...
    for pattern_name, pattern_config in patterns.items():
        ...
        if pattern_name == "phase_naming":
            if rel_path.startswith("phase_") and not re.match(
                pattern_config["pattern"], file_name
            ):
                compliance["errors"].append(
                    f"File name doesn't follow phase_XX_name.py pattern: {file_name}"
                )
            ...
            if not pattern_found:
                if required:
                    compliance["errors"].append(
                        f"Missing {pattern_config['description']}"
                    )
                    compliance["compliant"] = False
                else:
                    compliance["warnings"].append(
    ...
    if rel_path.startswith("scrapers/") or "phase_" in rel_path:
        if "BaseScraper" not in content:
            compliance["suggestions"].append(
                "Consider inheriting from BaseScraper for consistency"
            )
        if "config.yaml" not in content:
            compliance["suggestions"].append(
                "Consider loading configuration from config.yaml"
            )
...
def generate_compliance_report(patterns_data):
    ...
    for pattern_name, pattern_config in patterns_data["patterns"].items():
        ...
        report.append(
            f"- **Required:** {'Yes' if pattern_config.get('required', False) else 'No'}"
        )
        if pattern_config.get("required_for"):
            report.append(
                f"- **Required for:** {', '.join(pattern_config['required_for'])}"
            )
        report.append("")
    ...
    for file_path, file_info in sorted(patterns_data["files"].items()):
        ...
        report.append(
            f"- **Status:** {'✅ Compliant' if compliance.get('compliant', False) else '❌ Non-compliant'}"
        )
...
def main():
    ...
    for file_path in file_paths:
        ...
        compliance = check_pattern_compliance(
            file_path, content_info, INTELFORGE_PATTERNS
        )
    ...
    compliant_files = sum(
        1
        for info in patterns_data["files"].values()
