...
STDLIB_MODULES = {
    ...
    'functools', 'operator', 're', 'math', 'random', 'time', 'logging',
    'argparse', 'configparser', 'sqlite3', 'urllib', 'http', 'email',
    'xml', 'html', 'csv', 'pickle', 'gzip', 'zipfile', 'tarfile',
    'shutil', 'subprocess', 'threading', 'multiprocessing', 'queue',
    'socket', 'ssl', 'hashlib', 'hmac', 'secrets', 'base64', 'binascii',
    'struct', 'codecs', 'locale', 'calendar', 'uuid', 'enum', 'dataclasses',
    'typing', 'copy', 'pprint', 'warnings', 'traceback', 'inspect',
    'gc', 'weakref', 'types', 'abc', 'contextlib', 'importlib'
}
...
def generate_dependency_report(deps):
    ...
    stdlib_count = sum(1 for info in deps["files"].values() 
                      for imp in info["imports"] 
                      if imp["classification"] == "stdlib")
    third_party_count = sum(1 for info in deps["files"].values() 
                           for imp in info["imports"] 
                           if imp["classification"] == "third-party")
    local_count = sum(1 for info in deps["files"].values() 
                     for imp in info["imports"] 
    ...
    for file_path, info in sorted(deps["files"].items()):
        ...
        if info["imports"]:
            stdlib_imports = [imp for imp in info["imports"] if imp["classification"] == "stdlib"]
            third_party_imports = [imp for imp in info["imports"] if imp["classification"] == "third-party"]
            local_imports = [imp for imp in info["imports"] if imp["classification"] == "local"]
            
            if stdlib_imports:
                report.append(f"- **Standard library:** {', '.join(imp['module'] for imp in stdlib_imports)}")
            if third_party_imports:
                report.append(f"- **Third-party:** {', '.join(imp['module'] for imp in third_party_imports)}")
            if local_imports:
                report.append(f"- **Local:** {', '.join(imp['module'] for imp in local_imports)}")
        ...
        for imp in info["imports"]:
            if imp["classification"] == "unknown":
                ...
                if top_level and top_level not in STDLIB_MODULES:
                    warning = f"Unknown package '{top_level}' in {file_path}:{imp['line']}"
