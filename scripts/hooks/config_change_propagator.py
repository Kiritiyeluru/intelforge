...
def update_config_changelog(changes):
    ...
    for change in changes:
        if change["type"] == "added":
            ...
            new_entry.append(f"- **Removed:** `{change['path']}` (was `{change['value']}`)")
        elif change["type"] == "modified":
            new_entry.append(f"- **Modified:** `{change['path']}` from `{change['old_value']}` to `{change['new_value']}`")
...
def main():
    ...
    if changes:
        ...
        if dependencies:
            print(f"✓ Detected {len(dependencies)} implied dependencies: {', '.join(dependencies)}")
