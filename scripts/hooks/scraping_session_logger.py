...
def main():
    ...
    if session_info["metrics"].get("items_scraped"):
        history["summary"]["total_items_scraped"] += session_info["metrics"]["items_scraped"]
    ...
    with open(daily_log_file, 'a') as f:
        f.write(f"[{session_info['timestamp']}] {session_info['scraper_type']}: {command}\n")
        if session_info["metrics"]:
            f.write(f"  Metrics: {json.dumps(session_info['metrics'])}\n")
        f.write(f"  Dry run: {session_info['dry_run']}\n\n")
    
    print(f"✓ Scraping session logged: {session_info['scraper_type']} ({'dry-run' if session_info['dry_run'] else 'live'})")
