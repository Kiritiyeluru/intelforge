...
class CoverageAnalyzer:
    ...
    def run_coverage_analysis(self, test_markers: List[str] = None, 
                            exclude_markers: List[str] = None) -> Dict:
    ...
    def analyze_coverage_data(self, coverage_data: Optional[Dict]) -> Optional[Dict]:
        ...
        for category, data in category_analysis.items():
            if data['modules']:
                avg_coverage = sum(m.coverage_percent for m in data['modules']) / len(data['modules'])
    ...
    def generate_coverage_report(self, analysis_result: Dict) -> str:
        ...
        report.append("==========================")
        report.append(f"Overall Coverage: {overall['percent']:.1f}% {status_indicators.get(overall['status'], '❓')}")
        report.append(f"Lines: {overall['lines_covered']}/{overall['lines_total']}")
        report.append(f"Branches: {overall['branches_covered']}/{overall['branches_total']}")
        report.append("")
        ...
        for category, data in analysis['categories'].items():
            ...
            report.append(f"{category.title():>10}: {data['avg_coverage']:>5.1f}% {status_icon} "
