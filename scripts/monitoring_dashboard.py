...
class ScrapingLogger:
    ...
    def init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_timestamp ON scraping_events(timestamp)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_event_type ON scraping_events(event_type)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_scraper_type ON scraping_events(scraper_type)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_bot_detected ON scraping_events(bot_detected)"
            )
...
class PerformanceMonitor:
    ...
    def check_alerts(self, hours: int = 1) -> List[Dict[str, Any]]:
        ...
        total_requests = sum(
            stats["total"] for stats in metrics["success_rates"].values()
        )
        if total_requests > 0:
            bot_detection_rate = (
                metrics["bot_detection"]["count"] / total_requests
            ) * 100
...
class FinancialAnalytics:
    ...
    def generate_financial_dashboard_html(self, hours: int = 24) -> str:
        ...
        if "market_data" in market_context:
            ...
            changes = [
                market_context["market_data"][s].get("change_pct", 0) for s in symbols
            ]
            colors = ["green" if x > 0 else "red" for x in changes]

            fig.add_trace(
                go.Bar(
                    x=symbols, y=changes, marker_color=colors, name="Market Change %"
                ),
                row=1,
                col=1,
            )
        ...
        if financial_metrics["financial_sites_performance"]:
            ...
            success_rates = [
                financial_metrics["financial_sites_performance"][s]["success_rate"]
                for s in sites
            ]

            fig.add_trace(
                go.Bar(
                    x=sites, y=success_rates, name="Success Rate %", marker_color="blue"
                ),
                row=1,
                col=2,
            )
            ...
            # Execution times
            exec_times = [
                financial_metrics["financial_sites_performance"][s][
                    "avg_execution_time"
                ]
                for s in sites
            ]
            fig.add_trace(
                go.Scatter(
                    x=sites, y=exec_times, mode="markers+lines", name="Avg Time (s)"
                ),
                row=2,
                col=1,
            )
            ...
            fig.add_trace(
                go.Bar(
                    x=sites, y=bot_rates, name="Bot Detection %", marker_color="orange"
                ),
                row=2,
                col=2,
            )
...
"""
        ...
        total_successes = sum(
            stats["successes"] for stats in metrics["success_rates"].values()
        )
...
"""
        ...
        if avg_execution_time > 8:
            ...
            report += (
                "   🚀 Excellent performance - consider increasing concurrent workers\n"
            )
        ...
        try:
            while True:
                ...
                if alerts:
                    ...
                    print(
                        f"✅ [{datetime.now().strftime('%H:%M:%S')}] All systems normal"
                    )
...
def main():
    ...
    parser.add_argument(
        "--hours", type=int, default=24, help="Time period for report (hours)"
    )
    parser.add_argument(
        "--interval", type=int, default=300, help="Monitoring interval (seconds)"
    )
    parser.add_argument(
        "--alerts", action="store_true", help="Check current alerts only"
    )
    parser.add_argument(
        "--financial", action="store_true", help="Generate financial dashboard HTML"
    )
    parser.add_argument(
        "--market", action="store_true", help="Get current market context only"
    )
    ...
    if args.report:
        ...
        if "error" in market_context:
            ...
            print(
                f"🏦 Market Sentiment: {market_context.get('market_sentiment', 'UNKNOWN')}"
            )
