...
class EnhancedMonitoringDashboard:
    ...
    def generate_financial_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            fig = make_subplots(
                rows=1, cols=1,
                subplot_titles=("Market Performance Today")
            )
            ...
            # Save dashboard
            dashboard_path = self.reports_dir / f"financial_dashboard_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
    ...
    def _generate_market_summary(self, market_data: Dict[str, Any]) -> str:
        ...
        for symbol, data in market_data.items():
            ...
            summary_parts.append(f"{symbol}: {data['price']} ({direction} {abs(data['change_pct']):.2f}%)")
...
def main():
    ...
    parser.add_argument(
        "--market", 
        action="store_true",
        help="Generate market data report"
    )
    parser.add_argument(
        "--financial", 
        action="store_true",
    ...
    parser.add_argument(
        "--test", 
        action="store_true",
        help="Run in test mode (use mock data)"
    )
    parser.add_argument(
        "--all", 
        action="store_true",
