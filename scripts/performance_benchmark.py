    ...
    def run_html_parsing_benchmarks(self):
        ...
        if bs4_result["success"]:
            ...
            if selectolax_result["success"]:
                ...
                print(
                    f"    🚀 selectolax is {improvement:.1f}x faster than BeautifulSoup"
                )
        else:
    ...
    def print_summary(self):
        ...
        if summary["performance_gains"]:
            ...
            for gain in summary["performance_gains"]:
                print(
                    f"  • {gain['category']}: {gain['improvement']} ({gain['technology']})"
                )
        ...
        if summary["http_requests_comparison"]:
            if summary["http_requests_comparison"] > 1:
                print(
                    f"🌐 HTTP Requests: httpx is {summary['http_requests_comparison']:.1f}x faster than requests"
                )
            else:
                print(
                    "🌐 HTTP Requests: Similar performance between httpx and requests"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Performance Benchmark Suite for IntelForge"
    )
    parser.add_argument(
        "--detailed",
        action="store_true",
        help="Run detailed benchmarks including full scrapers",
    )
    parser.add_argument(
