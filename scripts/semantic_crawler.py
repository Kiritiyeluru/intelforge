...
def load_reference_vector():
    ...
    try:
        with open(REFERENCE_VECTOR_PATH, 'r') as f:
            data = json.load(f)
        print(f"✅ Loaded reference embeddings (created: {data.get('created_at', 'unknown')})")
...
def is_content_relevant(content: str, threshold: float = DEFAULT_THRESHOLD) -> tuple[bool, float]:
...
def embed_to_qdrant(markdown_file: str, metadata_file: str, data: Dict[str, Any], similarity_score: float):
...
def run_semantic_crawler(urls: List[str], threshold: float = DEFAULT_THRESHOLD, dry_run: bool = False):
    """Main crawler function with AI filtering"""
    
    print(f"🧠 Starting Semantic Crawler")
    ...
    for html, url in results:
        ...
        is_relevant, similarity_score = is_content_relevant(parsed_data["content"], threshold)
        ...
        if is_relevant:
            relevant_count += 1
            print(f"  ✅ RELEVANT - Capturing content")
            
            if not dry_run:
                # Save to markdown with metadata
                markdown_file, metadata_file = save_to_markdown(parsed_data, similarity_score)
                
                # Embed to Qdrant
                embed_to_qdrant(markdown_file, metadata_file, parsed_data, similarity_score)
            else:
    ...
    # Summary statistics
    print("\n" + "=" * 60)
    print(f"📊 SEMANTIC CRAWLER SUMMARY")
    print("=" * 60)
    print(f"URLs processed: {processed_count}")
    print(f"Content captured: {relevant_count}")
    print(f"Capture rate: {(relevant_count/processed_count*100):.1f}%" if processed_count > 0 else "N/A")
