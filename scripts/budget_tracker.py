...
class BudgetTracker:
    ...
    def __init__(self, project_root: Path):
        ...
        self.phase_budgets = {
            'Part_1_1_Outcome_Verification': {
                ...
                'tasks': ['fail_fast_validator', 'claude_integrity_validator', 'vector_health_validator', 'health_contract'],
                'priority': 'critical',
                'status': 'completed'
            },
            ...
            'Part_2_1_Snapshot_Drift': {
                ...
                'tasks': ['tolerance_config', 'snapshot_drift_validator', 'drift_reports'],
                'priority': 'high',
                'status': 'completed'
            },
            'Part_2_2_Performance_Benchmarking': {
                ...
                'tasks': ['cli_performance', 'performance_baseline', 'regression_framework'],
                'priority': 'high',
                'status': 'completed'
            },
            'Part_2_3_Enhanced_Module_Testing': {
                ...
                'tasks': ['ml_validation', 'embedding_stability', 'academic_integration'],
                'priority': 'high',
                'status': 'completed'
            },
            'Part_3A_Persona_Functionality': {
                ...
                'tasks': ['researcher_persona', 'trader_persona', 'developer_persona', 'e2e_workflows'],
                'priority': 'high',
                'status': 'completed'
            },
            'Part_3B_System_Hardening': {
                ...
                'tasks': ['k6_load_testing', 'test_tagging', 'coverage_analysis', 'budget_tracking'],
                'priority': 'medium',
                'status': 'in_progress'
            },
    ...
    def load_budget_data(self) -> Dict:
        ...
        return {
            ...
            'total_allocated_hours': sum(phase['allocated'] for phase in self.phase_budgets.values()),
        ...
        for alert_status, threshold, message in alert_conditions:
            if status == alert_status and spent >= allocated * threshold:
                alert = {
                    ...
                    'utilization_percent': (spent / allocated * 100) if allocated > 0 else 0,
                }
                
                # Only add if not already alerted for this condition
                existing_alerts = [a for a in self.budget_data['alerts'] 
                                 if a['phase'] == phase and a['status'] == status.value]
                if not existing_alerts:
                    self.budget_data['alerts'].append(alert)
                    print(f"🚨 BUDGET ALERT: {phase} - {message}")
                    print(f"   Spent: {spent:.1f}h / Allocated: {allocated:.1f}h ({spent/allocated*100:.1f}%)")
    ...
    def get_overall_budget_status(self) -> Tuple[BudgetStatus, Dict]:
        ...
        summary = {
            ...
            'phases_completed': len([p for p in self.phase_budgets.values() if p.get('status') == 'completed']),
    ...
    def generate_budget_report(self) -> str:
        ...
        report.append("=====================================")
        report.append(f"Overall Status: {status.value.title()} {status_indicators[status]}")
        report.append(f"Total Budget: {summary['total_allocated']:.1f} hours")
        report.append(f"Spent: {summary['total_spent']:.1f} hours ({summary['utilization_percent']:.1f}%)")
        report.append(f"Remaining: {summary['remaining']:.1f} hours")
        report.append(f"Phases Completed: {summary['phases_completed']}/{summary['phases_total']}")
        ...
        for phase_key, phase_config in self.phase_budgets.items():
            ...
            report.append(f"{phase_key:>30}: {spent:>5.1f}h / {allocated:>5.1f}h "
                         f"({utilization:>5.1f}%) {status_icon}")
            
            if status in [BudgetStatus.WARNING, BudgetStatus.OVER_BUDGET, BudgetStatus.CRITICAL]:
        ...
        if self.budget_data['alerts']:
            ...
            report.append("-----------------------")
            recent_alerts = sorted(self.budget_data['alerts'], 
            ...
            for alert in recent_alerts:
                ...
                report.append(f"{'':>20} Utilization: {alert['utilization_percent']:.1f}%")
    ...
    def budget_check(self, warn_threshold: float = 95.0) -> bool:
        ...
        if summary['utilization_percent'] >= warn_threshold:
            print(f"⚠️  Budget warning: {summary['utilization_percent']:.1f}% of budget used")
            print(f"   Spent: {summary['total_spent']:.1f}h / Allocated: {summary['total_allocated']:.1f}h")
    ...
    def generate_markdown_report(self):
        ...
        md_report = report.replace('=', '-').replace('💰', '##').replace('📋', '###').replace('🚨', '###').replace('💡', '###')
...
def main():
    ...
    parser.add_argument('--add-time', nargs=4, metavar=('PHASE', 'TASK', 'HOURS', 'DESCRIPTION'),
                       help='Add time entry: phase task hours description')
    parser.add_argument('--report', action='store_true',
                       help='Generate budget report')
    parser.add_argument('--check', action='store_true',
                       help='Check budget status')
    parser.add_argument('--warn-if-over', type=float, default=95.0,
    ...
    if not any([args.add_time, args.report, args.check, args.markdown]):
        ...
        print(f"Budget Status: {summary['utilization_percent']:.1f}% "
