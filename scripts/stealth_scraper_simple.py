...
def create_stealth_session() -> httpx.Client:
    ...
    if STEALTH_AVAILABLE:
        ...
        client = httpx.Client(
            headers=headers, timeout=30.0, follow_redirects=True, verify=True
        )
        return client
...
def stealth_scrape_url(url: str, session: httpx.Client, delay: int = 3) -> Dict:
    ...
    try:
        ...
        if bot_detected:
            ...
            print(
                f"   ✅ Stealth successful - {len(extracted['main_content']):,} chars"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Simple Stealth Scraper - HTTP Anti-Detection"
    )
    ...
    print("🕵️  Simple Stealth Scraper - HTTP Anti-Detection")
    print(f"📄 URLs to scrape: {len(urls)}")
    print(f"⏱️  Delay between requests: {args.delay}s")
    print(
        f"🛡️  Stealth method: {'stealth-requests' if STEALTH_AVAILABLE else 'httpx + realistic headers'}"
    )
    print("-" * 60)
    ...
    try:
        ...
        # Summary
        print(f"\n{'=' * 60}")
        print("📊 HTTP Stealth Scraping Summary:")
        ...
        print(
            f"   Stealth method: {'stealth-requests' if STEALTH_AVAILABLE else 'httpx + realistic headers'}"
        )
