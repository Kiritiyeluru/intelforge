...
class VectorSecurityManager:
    ...
    def _setup_encryption(self):
        ...
        if key_path.exists():
            ...
            # Generate new key
            password = os.environ.get('VECTOR_ENCRYPTION_PASSWORD', 'default_password').encode()
            salt = os.urandom(16)
            kdf = PBKDF2HMAC(
    ...
    def encrypt_vector_data(self, data: bytes) -> bytes:
        ...
        try:
            ...
            self.log_security_event("ENCRYPTION", "Vector data encrypted", {"data_size": len(data)})
            return encrypted_data
        except Exception as e:
            self.log_security_event("ENCRYPTION_ERROR", f"Encryption failed: {str(e)}", {"error": str(e)})
            raise
    ...
    def decrypt_vector_data(self, encrypted_data: bytes) -> bytes:
        ...
        try:
            ...
            self.log_security_event("DECRYPTION", "Vector data decrypted", {"data_size": len(decrypted_data)})
            return decrypted_data
        except Exception as e:
            self.log_security_event("DECRYPTION_ERROR", f"Decryption failed: {str(e)}", {"error": str(e)})
            raise
    ...
    def validate_vector_payload(self, payload: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if payload_size > 10 * 1024 * 1024:  # 10MB limit
                self.log_security_event("VALIDATION_FAILED", "Payload too large", {"size": payload_size})
                return False
            ...
            for pattern in suspicious_patterns:
                if pattern in payload_str.lower():
                    self.log_security_event("VALIDATION_FAILED", f"Suspicious pattern detected: {pattern}", 
                                           {"pattern": pattern})
                    return False
            ...
            if 'embedding' in payload:
                ...
                if isinstance(embedding, list) and len(embedding) > 0:
                    if len(embedding) > 2048:  # Reasonable embedding size limit
                        self.log_security_event("VALIDATION_FAILED", "Embedding too large", 
                                               {"size": len(embedding)})
                        return False
                    
                    # Check for malformed embeddings
                    if not all(isinstance(x, (int, float)) for x in embedding):
                        self.log_security_event("VALIDATION_FAILED", "Invalid embedding format", {})
                        return False
            
            self.log_security_event("VALIDATION_SUCCESS", "Payload validation passed", 
                                   {"size": payload_size})
            return True
            
        except Exception as e:
            self.log_security_event("VALIDATION_ERROR", f"Validation error: {str(e)}", {"error": str(e)})
            return False
    
    def log_security_event(self, event_type: str, message: str, metadata: Dict[str, Any] = None):
    ...
    def log_vector_operation(self, operation: str, collection: str, 
                           metadata: Dict[str, Any] = None, user: str = "system"):
    ...
    def check_access_permissions(self, user: str, operation: str, collection: str) -> bool:
        ...
        if operation.lower() in allowed_operations:
            self.log_security_event("ACCESS_GRANTED", f"Access granted for {operation}", 
                                   {"user": user, "operation": operation, "collection": collection})
            return True
        else:
            self.log_security_event("ACCESS_DENIED", f"Access denied for {operation}", 
                                   {"user": user, "operation": operation, "collection": collection})
            return False
    ...
    def generate_api_key(self, user: str, permissions: List[str]) -> str:
        ...
        self.log_security_event("API_KEY_GENERATED", f"API key generated for user {user}", 
    ...
    def validate_api_key(self, api_key: str) -> Optional[Dict[str, Any]]:
        ...
        self.log_security_event("API_KEY_VALIDATION", "API key validation requested", 
    ...
    def sanitize_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in metadata.items():
            ...
            if any(sensitive in key_lower for sensitive in sensitive_keys):
                ...
                self.log_security_event("METADATA_SANITIZED", f"Sensitive key sanitized: {key}", 
                                       {"key": key})
            else:
                sanitized[key] = value
    ...
    def get_security_health_check(self) -> Dict[str, Any]:
        ...
        health_status = {
            ...
            "key_management": {
                ...
                "status": "configured" if Path(self.config.secret_key_path).exists() else "not_configured"
            }
        }
        
        self.log_security_event("HEALTH_CHECK", "Security health check performed", health_status)
