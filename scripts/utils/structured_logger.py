...
class StructuredLogger:
        ...
        with self.context(
            ...
            **metadata
            ...
            try:
                ...
                logger.info(f"Completed operation: {operation_name} in {duration:.2f}s", extra={
                ...
                duration = time.time() - start_time
                logger.error(f"Operation failed: {operation_name} after {duration:.2f}s", extra={
    ...
    def _record_operation_metrics(self, operation_id: str, status: str, duration: float, error: str = None):
    ...
    def log_performance_data(self, 
        ...
        logger.info(f"Performance data for {operation}", extra={
...
def log_operation(operation_name: str, operation_type: str = "general", **metadata):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            structured_logger = get_structured_logger()
            with structured_logger.operation(operation_name, operation_type, **metadata):
...
if __name__ == "__main__":
    ...
    with structured_logger.context(user_id="user123", session_type="demo"):
        ...
        # Operation tracking
        with structured_logger.operation("data_processing", "analysis", dataset="example"):
