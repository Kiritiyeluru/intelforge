...
class BasicPIIDetector:
    ...
    def __init__(self):
        self.patterns = {
            ...
            PIIType.URL: re.compile(r'https?://(?:[-\w.])+(?:\:[0-9]+)?(?:/(?:[\w/_.])*(?:\?(?:[\w&=%.])*)?(?:\#(?:[\w.])*)?)?'),
        }
    ...
    def __init__(self, use_presidio: bool = True):
        ...
        if self.use_presidio:
            try:
                ...
                logger.warning(f"Failed to initialize Presidio: {e}, falling back to basic detector")
        ...
        try:
            ...
            results = self.analyzer.analyze(
                ...
                entities=["PERSON", "EMAIL_ADDRESS", "PHONE_NUMBER", "CREDIT_CARD", 
                         "IP_ADDRESS", "LOCATION", "DATE_TIME", "URL", "US_SSN"]
            )
...
def check_content_safety(content: str, risk_threshold: float = 50.0) -> Tuple[bool, float]:
