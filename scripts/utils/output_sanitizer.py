...
class OutputSanitizer:
    ...
    def __init__(self, strict_mode: bool = False):
        ...
        self.security_patterns = [
            ...
            (r'(?i)(password|passwd|pwd|secret|key|token)\s*[=:]\s*["\']?[a-zA-Z0-9_\-!@#$%^&*()]{8,}["\']?', '[REDACTED_CREDENTIAL]'),
            ...
            (r'/(?:home|users)/[a-zA-Z0-9_\-]+(?:/[a-zA-Z0-9_\-./]+)?', '[PATH_REDACTED]'),
        ]
    ...
    def sanitize_text(self, text: str, preserve_formatting: bool = True) -> str:
        ...
        for pattern in self.injection_patterns:
            text = re.sub(pattern, '[SCRIPT_REMOVED]', text, flags=re.IGNORECASE | re.DOTALL)
    ...
    def sanitize_json_output(self, data: Any, max_depth: int = 10) -> Dict[str, Any]:
        ...
        if isinstance(data, dict):
            ...
            return [self.sanitize_json_output(item, max_depth - 1) for item in data[:1000]]  # Limit list size
    ...
    def create_safe_output_report(self, data: Any, title: str = "IntelForge Output") -> Dict[str, Any]:
        """
    ...
    def get_sanitization_stats(self, original: str, sanitized: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "reduction_percentage": round((1 - len(sanitized) / max(len(original), 1)) * 100, 2),
            "lines_original": original.count('\n') + 1,
            "lines_sanitized": sanitized.count('\n') + 1,
            "characters_removed": len(original) - len(sanitized),
            "contains_redactions": '[REDACTED' in sanitized or '[SCRIPT_REMOVED]' in sanitized
        }
