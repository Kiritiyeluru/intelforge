...
class PerformanceMonitor:
    ...
    def __init__(self, 
        ...
        self.data_dir = data_dir or Path(__file__).parent.parent.parent / "logs" / "performance"
        ...
        if self.enable_verbose:
            self.logger.info("Performance monitoring initialized", extra={
    ...
    def _get_current_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "system_memory_available_gb": round(sys_memory.available / (1024**3), 2),
                "disk_io": disk_io,
    ...
    def _log_operation_results(self, metric: PerformanceMetric):
        ...
        if metric.memory_mb_after and metric.memory_mb_after > self.thresholds['memory_mb_critical']:
            log_level = "ERROR"
            warnings.append(f"Memory usage critical: {metric.memory_mb_after:.1f}MB")
        elif metric.memory_mb_after and metric.memory_mb_after > self.thresholds['memory_mb_warning']:
            if log_level == "INFO":
                log_level = "WARNING"
    ...
    def _save_metric_to_file(self, metric: PerformanceMetric):
        ...
        try:
            metrics_file = self.data_dir / f"performance_metrics_{datetime.now().strftime('%Y%m%d')}.jsonl"
    ...
    def get_operation_statistics(self, operation_name: str = None) -> Dict[str, Any]:
        ...
        for op_name, op_metrics in operations.items():
            ...
            memory_deltas = [m.memory_usage_delta_mb for m in op_metrics if m.memory_usage_delta_mb]
            ...
            stats[op_name] = {
                ...
                "exception_count": sum(1 for m in op_metrics if m.custom_metrics.get("exception")),
                "avg_performance_score": sum(self._calculate_performance_score(m) for m in op_metrics) / len(op_metrics)
            }
    ...
    def export_performance_report(self, 
        ...
        report_data = {
            ...
            "recent_metrics": [asdict(m) for m in list(self.metrics_history)[-50:]],  # Last 50 operations
            "thresholds": self.thresholds
        }
    ...
    def _export_markdown_report(self, export_path: Path, report_data: Dict[str, Any]):
        ...
        with open(export_path, 'w') as f:
            ...
            f.write(f"**Total Operations:** {report_data['metadata']['total_operations']}\n\n")
            ...
            if 'error' not in baseline:
                ...
                f.write(f"- **Memory Total:** {baseline.get('memory_total_gb', 'N/A')} GB\n")
                f.write(f"- **Disk Total:** {baseline.get('disk_total_gb', 'N/A')} GB\n\n")
            
            # Operation statistics
            stats = report_data['operation_statistics']
            if stats:
                f.write("## Operation Statistics\n\n")
                f.write("| Operation | Executions | Avg Duration (s) | Avg Memory (MB) | Performance Score |\n")
                f.write("|-----------|------------|------------------|-----------------|-------------------|\n")
                
                for op_name, op_stats in stats.items():
                    f.write(f"| {op_name} | {op_stats['total_executions']} | "
