...
class FreshnessTracker:
    ...
    def update_domain_metrics(self, domain: str) -> FreshnessMetrics:
        ...
        fresh_count = sum(1 for r in records if r["status"] == FreshnessStatus.FRESH.value)
        stale_count = sum(1 for r in records if r["status"] == FreshnessStatus.STALE.value)
        expired_count = sum(1 for r in records if r["status"] == FreshnessStatus.EXPIRED.value)
    ...
    def get_overall_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success_rate = (totals["success_count"] / totals["total_urls"]) * 100 if totals["total_urls"] > 0 else 0
            
            # Get domain-specific metrics
            domain_metrics = list(self.db.execute("""
    ...
    def generate_freshness_report(self, format_type: str = "table", days: int = 7) -> str:
...
def determine_freshness_status(last_crawled: datetime, fresh_threshold_hours: int = 6, 
                             stale_threshold_hours: int = 24) -> FreshnessStatus:
    ...
    for domain in domains:
        for i in range(random.randint(5, 15)):
            ...
            record = CrawlRecord(
                ...
                metadata={"user_agent": "IntelForge/1.0", "retry_count": random.randint(0, 2)}
            )
