...
class GracefulShutdown:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None):
        ...
        self.cleanup_status = {"total_hooks": 0, "successful": 0, "failed": 0, "errors": []}
    ...
    def _register_signal_handlers(self):
        ...
        try:
            ...
            self.logger.info("Graceful shutdown handlers registered for SIGINT and SIGTERM")
    ...
    def register_active_operation(self, operation_id: str, operation_data: Dict[str, Any]):
    ...
    def _wait_for_active_operations(self):
        ...
        while time.time() - start_time < max_wait:
            ...
            self.logger.info(f"Waiting for {len(active_ops)} active operations: {active_ops}")
            time.sleep(1)
        else:
            active_ops = [
                op_id for op_id, op_data in self.active_operations.items()
            ...
            if active_ops:
                self.logger.warning(f"Timeout waiting for operations. Force-completing: {active_ops}")
    ...
    def _execute_cleanup_hooks(self):
        ...
        success_rate = (self.cleanup_status["successful"] / self.cleanup_status["total_hooks"]) * 100
        ...
        report = {
            ...
            "active_operations": {
                op_id: {
                    ...
                    "duration": op_data.get("end_time", time.time()) - op_data["start_time"]
                }
                for op_id, op_data in self.active_operations.items()
            },
            "status": "SUCCESS" if self.cleanup_status["failed"] == 0 else "PARTIAL_SUCCESS"
        }
    ...
    def operation_context(self, operation_id: str, **operation_data):
        ...
        try:
            if self.shutdown_event.is_set():
                raise InterruptedError("Shutdown in progress, cannot start new operation")
            yield
        finally:
...
def setup_cli_shutdown_handling(logger: Optional[logging.Logger] = None):
    ...
    handler.register_cleanup_hook(
        lambda: logging.info("CLI shutdown complete"),
        "cli_final_log"
    )
