...
class TTRTracker:
    ...
    def create_incident(self,
        ...
        self.logger.info(f"Created incident {incident_id}: {title}", extra={
            "incident_id": incident_id,
    ...
    def update_incident_status(self,
        ...
        self.logger.info(f"Updated incident {incident_id} status: {old_status.value} -> {status.value}", extra={
            "incident_id": incident_id,
    ...
    def add_recovery_action(self,
        ...
        self.logger.info(f"Added recovery action to {incident_id}: {action}", extra={
            "incident_id": incident_id,
            "action": action
        })
    ...
    def resolve_incident(self,
        ...
        self.logger.info(f"Resolved incident {incident_id} in {ttr:.1f} minutes (SLA: {sla_met})", extra={
            "incident_id": incident_id,
            "ttr_minutes": ttr,
    ...
    def get_metrics_summary(self, days: int = 30) -> Dict[str, Any]:
        ...
        for severity in IncidentSeverity:
            ...
            if severity_incidents:
                by_severity[severity.value] = {
                    ...
                    "avg_ttr_minutes": sum(inc.ttr_minutes for inc in severity_incidents) / len(severity_incidents),
                    "target_minutes": self.ttr_targets.get(severity, 0),
                    "sla_compliance_rate": (
                        sum(1 for inc in severity_incidents 
                            if inc.ttr_minutes <= self.ttr_targets.get(severity, float('inf'))) 
        ...
        for component in components:
            component_incidents = [inc for inc in recent_incidents if inc.component == component]
            by_component[component] = {
                "count": len(component_incidents),
                "avg_ttr_minutes": sum(inc.ttr_minutes for inc in component_incidents) / len(component_incidents)
            }
    ...
    def export_to_csv(self, filename: Optional[str] = None) -> Path:
        ...
        with open(export_path, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                'incident_id', 'title', 'description', 'severity', 'component',
                'detection_time', 'resolution_time', 'status', 'ttr_minutes',
                'sla_target_minutes', 'sla_met', 'root_cause', 'tags'
            ]
            ...
            for incident in all_incidents:
                target = self.ttr_targets.get(incident.severity, 0)
                sla_met = (incident.ttr_minutes <= target) if incident.ttr_minutes else None
                ...
                writer.writerow({
                    ...
                    'resolution_time': incident.resolution_time.isoformat() if incident.resolution_time else '',
    ...
    def export_to_markdown(self, filename: Optional[str] = None, days: int = 30) -> Path:
        """
        ...
        with open(export_path, 'w', encoding='utf-8') as f:
            f.write(f"# Time-to-Recovery Report\n\n")
            ...
            if metrics['by_severity']:
                ...
                f.write("| Severity | Count | Avg TTR (min) | Target (min) | SLA Compliance |\n")
                f.write("|----------|-------|---------------|--------------|----------------|\n")
                for severity, data in metrics['by_severity'].items():
                    f.write(f"| {severity.title()} | {data['count']} | {data['avg_ttr_minutes']:.1f} | {data['target_minutes']} | {data['sla_compliance_rate']:.1f}% |\n")
                f.write("\n")
            ...
            if metrics['by_component']:
                ...
                for component, data in metrics['by_component'].items():
                    f.write(f"| {component} | {data['count']} | {data['avg_ttr_minutes']:.1f} |\n")
                f.write("\n")
            ...
            if active_incidents:
                ...
                for incident in active_incidents:
                    ...
                    f.write(f"- **Detection Time:** {incident.detection_time.strftime('%Y-%m-%d %H:%M:%S')}\n")
    ...
    def _save_incidents(self):
        ...
        data = {
            'active': [self._incident_to_dict(inc) for inc in self.active_incidents.values()],
    ...
    def _save_ttr_metrics(self, incident: Incident):
        ...
        with open(self.metrics_file, 'a', newline='') as csvfile:
            fieldnames = [
                'timestamp', 'incident_id', 'severity', 'component',
                'ttr_minutes', 'sla_target_minutes', 'sla_met'
            ]
...
def create_incident(title: str, description: str, severity: str, component: str, tags: List[str] = None) -> str:
...
def resolve_incident(incident_id: str, root_cause: str = None, prevention_measures: List[str] = None) -> bool:
        ...
        try:
            ...
            if incident_id:
                ...
                tracker.resolve_incident(
                    ...
                    prevention_measures=["Add better error handling", "Improve input validation"]
                )
