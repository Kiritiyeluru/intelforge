...
class DiscoveryEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Set up user agent
        self.session.headers.update({"User-Agent": "IntelForge Discovery Engine 1.0"})
    ...
    def _initialize_clients(self):
        ...
        try:
            # GitHub API
            github_token = self.config.get("github", {}).get(
                "access_token"
            ...
            reddit_secret = reddit_config.get("client_secret") or os.getenv(
            ...
            if reddit_id and reddit_secret:
                ...
                self.reddit_client = praw.Reddit(
                    ...
                    user_agent=reddit_config.get(
                        "user_agent", "IntelForge Discovery Engine"
                    ),
                )
            ...
            for subreddit_name in subreddits:
                try:
                    ...
                    search_results = subreddit.search(
                        topic, limit=limit // len(subreddits), sort="relevance"
                    )
    ...
    def discover_all_sources(
        self, topic: str, sources: List[str], limit_per_source: int = 20
    ) -> List[Dict[str, Any]]:
    ...
    def save_discovery_results(
        self, discoveries: List[Dict[str, Any]], topic: str, output_dir: Path
    ):
        """Save discovery results to file"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        results_file = (
            output_dir / f"discovery_{topic.replace(' ', '_')}_{timestamp}.json"
        )
...
def main():
    ...
    parser.add_argument(
        "--topic", "-t", required=True, help="Topic to discover content for"
    )
    parser.add_argument(
        "--sources",
        "-s",
    ...
    parser.add_argument(
        "--min-score", type=float, default=0.3, help="Minimum relevance score filter"
    )
    ...
    filtered_discoveries = [
        d for d in discoveries if d.get("relevance_score", 0) >= args.min_score
    ]
    ...
    print(
        "  Score content: python scripts/llm_content_scorer.py --directory vault/notes/"
    )
