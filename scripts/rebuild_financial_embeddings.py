...
class FinancialAIProcessor:
    ...
    def chunk_text(self, text: str, source_file: str) -> List[Dict]:
        ...
        for i in range(0, text_length, CHUNK_SIZE - CHUNK_OVERLAP):
            ...
            chunk_id = hashlib.md5(
                f"{source_file}_{i}_{chunk_text[:50]}".encode()
            ).hexdigest()[:16]
    ...
    def build_embeddings(self):
        ...
        self.log_action(
            f"Generated {len(all_texts)} text chunks from {len(md_files)} files"
        )
        ...
        self.log_action(
            f"Average chunk size: {total_chars // len(all_chunks)} characters"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Financial AI Processor - Embeddings & Search"
    )
    ...
    if args.build:
        ...
        print(
            f"   💾 Vector DB size: {processor.vector_db_path.stat().st_size / 1024 / 1024:.1f} MB"
        )
