...
def run_academic_query(
    query: str, database: str = "arxiv", limit: int = 5
) -> Dict[str, Any]:
...
def test_concurrent_academic_research():
    ...
    for query, database, limit in academic_queries:
        ...
        print(
            f"  ✅ {query[:30]:<30} | {result['execution_time']:.2f}s | {'SUCCESS' if result['success'] else 'FAILED'}"
        )
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        for future in futures:
            ...
            print(
                f"  ✅ {result['query'][:30]:<30} | {result['execution_time']:.2f}s | {'SUCCESS' if result['success'] else 'FAILED'}"
            )
...
def test_concurrent_web_scraping():
    ...
    for url in test_urls:
        ...
        sequential_results.append(result)
        print(
            f"  ✅ {url:<35} | {result['execution_time']:.2f}s | {'SUCCESS' if result['success'] else 'FAILED'}"
        )
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        for future in futures:
            ...
            concurrent_results.append(result)
            print(
                f"  ✅ {result['url']:<35} | {result['execution_time']:.2f}s | {'SUCCESS' if result['success'] else 'FAILED'}"
            )
...
def main():
    ...
    print(
        f"   Efficiency Rating: {'EXCELLENT' if avg_speedup > 2.5 else 'GOOD' if avg_speedup > 1.5 else 'MODERATE'}"
    )
