...
def generate_test_html():
    ...
    """
        * 500
    )  # Repeat to make it larger for meaningful timing
...
def main():
    ...
    if selectolax_result["success"] and bs4_result["success"]:
        improvement = bs4_result["time"] / selectolax_result["time"]
        print(
            f"⚡ HTML Parsing: selectolax is {improvement:.1f}x faster than BeautifulSoup"
        )
    ...
    if httpx_result["success"] and requests_result["success"]:
        ...
        if ratio > 1:
            ...
            print(
                f"🌐 HTTP Requests: Similar performance (requests {1 / ratio:.1f}x faster)"
            )
    ...
    try:
        ...
        print("💾 Results saved to: vault/logs/simple_performance_test.json")
    except Exception as e:
