...
class ScrapingScheduler:
    ...
    def setup_schedules(self):
        ...
        if self.schedule_config["reddit"]["enabled"]:
            schedule.every().day.at(self.schedule_config["reddit"]["time"]).do(
                self.run_reddit_scraper
            )
            self.logger.info(
                f"Reddit scraper scheduled daily at {self.schedule_config['reddit']['time']}"
            )

        # GitHub scraper - weekly on Monday at 10 AM
        if self.schedule_config["github"]["enabled"]:
            schedule.every().monday.at(self.schedule_config["github"]["time"]).do(
                self.run_github_scraper
            )
            self.logger.info(
                f"GitHub scraper scheduled weekly on Monday at {self.schedule_config['github']['time']}"
            )

        # Web scraper - daily at 2 PM
        if self.schedule_config["web"]["enabled"]:
            schedule.every().day.at(self.schedule_config["web"]["time"]).do(
                self.run_web_scraper
            )
            self.logger.info(
                f"Web scraper scheduled daily at {self.schedule_config['web']['time']}"
            )
    ...
    def _show_next_jobs(self):
        ...
        if jobs:
            ...
            for job in jobs:
                self.logger.info(
                    f"  - {job.job_func.__name__}: next run at {job.next_run}"
                )
        else:
...
def main():
    ...
    parser.add_argument(
        "--crontab", action="store_true", help="Generate crontab entries and exit"
    )
    parser.add_argument(
        "--test", action="store_true", help="Test run with dry-run mode"
    )
    ...
    try:
        ...
        if args.crontab:
            ...
            print()
            print(
                "Note: Make sure to set up the full environment (PATH, etc.) in your crontab"
            )
