...
class ProductionReadinessChecker:
    ...
    def __init__(self, project_root: Optional[Path] = None, quick_mode: bool = False):
    ...
    def run_command(
        self, command: List[str], timeout: int = 30, capture_output: bool = True
    ) -> Tuple[int, str, str]:
    ...
    def check_security_baseline(self) -> ReadinessCheck:
        ...
        security_test_file = (
            self.project_root / "tests/security/test_security_baseline.py"
        )
        ...
        # Run security tests with appropriate timeout (45s - allows for AI model loading)
    ...
    def check_test_coverage(self) -> ReadinessCheck:
        ...
        if self.quick_mode:
            # Skip coverage analysis in quick mode
            return ReadinessCheck(
                name="Test Coverage",
                status="skip",
                score=75,
                message="Skipped in quick mode",
                recommendations=["Run full check for detailed coverage analysis"],
            )
        ...
        # Run coverage analysis with appropriate timeout (allows for AI model loading)
        exit_code, stdout, stderr = self.run_command(
            ["python", str(coverage_analyzer), "--quick"], timeout=90
        )

        if exit_code == 0:
            # Try to extract coverage percentage from output
            coverage_percent = 0
            coverage_file = self.project_root / "reports/coverage.json"
            ...
            if coverage_percent >= 5:  # Realistic threshold for quick tests
                ...
                status = "fail"
                score = max(30, int(coverage_percent * 6))
            ...
            return ReadinessCheck(
                ...
                recommendations=(
                    ["Improve test coverage to 80%+"] if coverage_percent < 80 else []
                ),
            )
        else:
            return ReadinessCheck(
    ...
    def check_performance_benchmarks(self) -> ReadinessCheck:
        ...
        if self.quick_mode:
            ...
            return ReadinessCheck(
                ...
                score=70,
                message="Skipped in quick mode",
                recommendations=["Run full check for performance analysis"],
            )

        perf_test_file = (
            self.project_root / "tests/performance/test_performance_regression.py"
        )
        ...
        # Run quick performance test with reduced timeout
        exit_code, stdout, stderr = self.run_command(
            ["python", str(perf_test_file), "--cli-only"], timeout=45
        )
    ...
    def check_cli_functionality(self) -> ReadinessCheck:
        ...
        # Test basic CLI commands with reduced timeout
        cli_commands = [
            (["python", "-m", "scripts.cli", "--help"], "CLI help"),
        ]

        # Skip status command in quick mode or add it with very short timeout
    ...
    def check_documentation(self) -> ReadinessCheck:
        ...
        if missing_docs:
            recommendations.append(
                f"Create missing documentation: {', '.join(missing_docs)}"
            )
        if not has_session_docs:
    ...
    def check_infrastructure_health(self) -> ReadinessCheck:
        ...
        recommendations = (
            [f"Fix: {item}" for item in failed_items] if failed_items else []
        )
    ...
    def check_budget_status(self) -> ReadinessCheck:
        ...
        if self.quick_mode:
            # Skip budget check in quick mode
            return ReadinessCheck(
                name="Budget Tracking",
                status="skip",
                score=70,
                message="Skipped in quick mode",
                recommendations=["Run full check for budget analysis"],
            )
        ...
        # Run budget check with reduced timeout
        exit_code, stdout, stderr = self.run_command(
            ["python", "-m", "scripts.cli", "budget-check"], timeout=15
        )

        if exit_code == 0:
            # Parse budget status from output
            over_budget = (
                "over budget" in stdout.lower() or "exceeded" in stdout.lower()
            )
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if failed_checks:
            recommendations.append(
                f"Address {len(failed_checks)} failing checks before deployment"
            )

        if warning_checks:
            recommendations.append(
                f"Review {len(warning_checks)} warning checks for production optimization"
            )
    ...
    def run_all_checks(self) -> ReadinessReport:
        ...
        if self.quick_mode:
            print("⚡ Quick mode enabled - skipping long-running checks")
        ...
        for i, check_method in enumerate(check_methods, 1):
            check_name = (
                check_method.__name__.replace("check_", "").replace("_", " ").title()
            )
            print(f"  [{i}/{len(check_methods)}] {check_name}...")
            try:
                ...
                print(
                    f"    {status_icon} {check_result.message} (Score: {check_result.score})"
                )
    ...
    def save_report(
        self, report: ReadinessReport, output_file: Optional[Path] = None
    ) -> Path:
        """Save report to JSON file."""
        if output_file is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_file = (
                self.project_root / "logs" / f"production_readiness_{timestamp}.json"
            )
...
def main():
    ...
    def timeout_handler(signum, frame):
        print("\n⏰ Production readiness check timed out after 90 seconds")
        print("💡 Try using --quick mode for faster results")
        sys.exit(1)
    ...
    try:
        parser = argparse.ArgumentParser(
            description="IntelForge Production Readiness Checker"
        )
        ...
        parser.add_argument(
            "--quick", action="store_true", help="Quick mode - skip long-running checks"
        )

        args = parser.parse_args()

        # Initialize checker
        checker = ProductionReadinessChecker(args.project_root, quick_mode=args.quick)
        ...
        print("\n⏹️ Production readiness check interrupted")
        sys.exit(1)
    except Exception as e:
        signal.alarm(0)
