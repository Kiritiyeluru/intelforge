...
try:
    from .claude_integrity_validator import ClaudeIntegrityValidator, validate_claude_output_cli
    CLAUDE_VALIDATOR_AVAILABLE = True
except ImportError:
    CLAUDE_VALIDATOR_AVAILABLE = False

try:
    from .vector_health_validator import VectorHealthValidator, quick_vector_health_check
...
class FailFastValidator:
    ...
    def _check_claude_output_integrity(self):
        ...
        if logs_dir.exists():
            recent_logs = sorted(logs_dir.glob("*.log"), key=lambda x: x.stat().st_mtime, reverse=True)[:5]
            potential_claude_files.extend(recent_logs)
        
        # Check data directory for recent files
        data_dir = self.project_root / "data"
        if data_dir.exists():
            recent_data = sorted(data_dir.glob("*.json"), key=lambda x: x.stat().st_mtime, reverse=True)[:3]
        ...
        if session_docs.exists():
            recent_docs = sorted(session_docs.glob("*.md"), key=lambda x: x.stat().st_mtime, reverse=True)[:3]
        ...
        try:
            ...
            if failed_files > 0:
                self.checks.append(ValidationCheck(
                    ...
                    details={"failed_files": failed_files, "total_files": total_files},
                ...
                self.checks.append(ValidationCheck(
                    ...
                    details={"suspicious_files": suspicious_files, "total_files": total_files}
                ))
            else:
                self.checks.append(ValidationCheck(
                    name="claude_output_integrity",
        ...
        try:
            ...
            if critical_issues > 0:
                self.checks.append(ValidationCheck(
                    ...
                    details={"critical_issues": critical_issues, "warning_issues": warning_issues},
...
def cli_fail_fast_check(command: str = "general") -> bool:
    ...
    critical_failures = [
        ...
        if check.level == ValidationLevel.CRITICAL and check.result == ValidationResult.FAIL
    ]
...
if __name__ == "__main__":
    ...
    if args.json:
        ...
        print(f"🔧 IntelForge Fail-Fast Validation Report")
        print(f"=" * 50)
        ...
        if args.verbose:
            ...
            for check in validator.checks:
                ...
                print(f"{icon} [{check.level.value.upper()}] {check.name}: {check.message}")
                if check.fix_suggestion and check.result in [ValidationResult.FAIL, ValidationResult.WARN]:
