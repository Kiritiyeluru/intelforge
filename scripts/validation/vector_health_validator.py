...
class VectorHealthValidator:
        ...
        try:
            ...
            if empty_percentage > 20:
                checks.append(HealthCheck(
                    ...
                    metrics={"empty_percentage": empty_percentage, "total_count": total_count},
    ...
    def _validate_qdrant_collection(self, collection_path: Path) -> List[HealthCheck]:
        ...
        try:
            ...
            data_files = [f for f in collection_files if f.is_file() and f.suffix in ['.bin', '.dat']]
            ...
            if not data_files:
                ...
                if total_size < 1024:  # Less than 1KB
                    checks.append(HealthCheck(
                        ...
                        metrics={"size_bytes": total_size, "file_count": len(data_files)}
                    ))
                else:
                    checks.append(HealthCheck(
                        name=f"qdrant_collection_size_{collection_path.name}",
    ...
    def _validate_embedding_dimensions(self, embeddings_data: Dict) -> List[HealthCheck]:
        ...
        if len(dimensions) > 1:
            checks.append(HealthCheck(
                ...
                metrics={"dimensions": list(dimensions), "valid_count": valid_embeddings},
            ...
            is_expected = any(dimension == exp_dim for exp_dim in self.expected_dimensions.values())
            ...
            if is_expected:
                checks.append(HealthCheck(
                    ...
                    metrics={"dimension": dimension, "valid_count": valid_embeddings}
                ))
            else:
                checks.append(HealthCheck(
                    name="embeddings_dimensions",
    ...
    def _assess_embedding_quality(self, embeddings_data: Dict) -> List[HealthCheck]:
        ...
        try:
            ...
            if low_variance_count > sample_size * 0.5:
                quality_score -= 0.4
                issues.append(f"{low_variance_count}/{sample_size} embeddings have very low variance")
            
            if avg_magnitude < 0.1:
                quality_score -= 0.3
                issues.append(f"Average embedding magnitude is very low: {avg_magnitude:.4f}")
            elif avg_magnitude > 100:
                quality_score -= 0.2
                issues.append(f"Average embedding magnitude is very high: {avg_magnitude:.2f}")
    ...
    def get_health_summary(self, checks: List[HealthCheck]) -> Dict[str, Any]:
        ...
        return {
            ...
            "critical_issues": [
                {"name": c.name, "message": c.message, "recommendation": c.recommendation}
                for c in checks
...
if __name__ == "__main__":
    ...
    parser.add_argument("--component", choices=["all", "chromadb", "qdrant", "embeddings"], 
    ...
    if args.json:
        ...
        print(f"🔍 Vector Store Health Report")
        print(f"=" * 40)
