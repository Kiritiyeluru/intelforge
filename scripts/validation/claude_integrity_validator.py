...
class ClaudeIntegrityValidator:
    ...
    def validate_claude_output(
        self, output: str, expected_format: str = "text", source: str = "claude"
    ) -> List[IntegrityCheck]:
    ...
    def _detect_gibberish(self, text: str) -> float:
        ...
        consonants = sum(
            1 for char in text_lower if char.isalpha() and char not in "aeiou"
        )
    ...
    def validate_file_output(
        self, file_path: Path, expected_format: str = "auto"
    ) -> List[IntegrityCheck]:
        ...
        try:
            ...
            checks.extend(
                self.validate_claude_output(content, expected_format, str(file_path))
            )
    ...
    def get_integrity_summary(self, checks: List[IntegrityCheck]) -> Dict[str, Any]:
        ...
        results = {
            ...
            "suspicious": sum(
                1 for c in checks if c.result == IntegrityResult.SUSPICIOUS
            ),
            "corrupted": sum(
                1 for c in checks if c.result == IntegrityResult.CORRUPTED
            ),
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--format", default="auto", help="Expected format (yaml, json, text, auto)"
    )
    ...
    if args.json:
        ...
        print("🔍 Claude Integrity Validation Report")
        print("=" * 45)
