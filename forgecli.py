...
@click.option('--output', '-o', default='vault/notes/pipeline/', help='Output directory')
@click.option('--smart-filter', is_flag=True, help='Use AI-powered semantic filtering during scraping')
def pipeline(topic: str, sources: tuple, limit: int, workers: int, model: str, threshold: float, output: str, smart_filter: bool):
...
@click.option('--sources', '-s', multiple=True, default=['google', 'github', 'arxiv'], 
              help='Sources to search: google, github, arxiv, reddit')
@click.option('--limit', '-l', default=20, help='Maximum results per source')
@click.option('--output', '-o', default='vault/notes/discovery/', help='Output directory')
...
def search(query: str, limit: int, threshold: float):
    ...
    search_cmd = [
        "python", "scripts/phase_08_ai_processor.py",
        "--search", query,
        "--limit", str(limit),
        "--threshold", str(threshold)
    ]
...
@cli.command()
@click.option('--url-file', '-f', required=True, help='File containing URLs to crawl (one per line)')
@click.option('--threshold', '-t', default=0.75, help='Similarity threshold for content relevance')
@click.option('--dry-run', is_flag=True, help='Preview mode - analyze content but don\'t save anything')
@click.option('--regenerate-reference', is_flag=True, help='Regenerate reference embeddings from training data')
def smart_crawl(url_file: str, threshold: float, dry_run: bool, regenerate_reference: bool):
    ...
    try:
        result = run_subprocess(cmd, f"AI-filtered semantic crawling (threshold: {threshold})")
        if result.returncode != 0:
            sys.exit(1)
    except Exception:
        sys.exit(1)
...
@click.option('--filter-method', type=click.Choice(['cosine', 'gpt'], case_sensitive=False), default='cosine', help='AI filtering method to use')
@click.option('--threshold', '-t', default=0.75, help='Minimum score required to capture content')
@click.option('--dry-run', is_flag=True, help='Preview mode - analyze content but don\'t save anything')
def ingest(url: str, filter_method: str, threshold: float, dry_run: bool):
    ...
    try:
        ...
        # Execute ingestion
        result = run_subprocess(cmd, f"Ingesting single URL with {filter_method} filtering")
...
@click.option('--sort', type=click.Choice(['score', 'date', 'title'], case_sensitive=False), default='score', help='Sort results by field')
...
def list(tag: str, min_score: float, max_results: int, sort: str, reverse: bool):
    ...
    click.echo(f"📊 Found: {len(items)} notes (showing {len(displayed_items)}/{total_notes} total)")
    ...
    for i, item in enumerate(displayed_items, 1):
        ...
        # Show metadata
        click.echo(f"    📊 {item['content_length']} chars | {item['filter_method']} | {item['captured_at'][:10]}")
    ...
    if len(items) > max_results:
        ...
        click.echo(f"💡 Use --max-results to show more")
...
def test(quick: bool, deep: bool, fix: bool):
    ...
    try:
        result = run_subprocess(["python", "--version"], "Checking Python version", capture_output=True)
        if result.returncode == 0:
    ...
    if deep:
        ...
        try:
            ...
            click.echo(f"  ✅ Memory usage: {memory.percent}% of {memory.total // (1024**3)}GB")
...
def status():
    ...
    try:
        ...
        result = run_subprocess([
            ...
            "run", "-n", "intelforge-py310", 
            "python", "--version"
        ], "Checking Python 3.10 environment", capture_output=True)
        if result.returncode == 0:
    ...
    try:
        result = run_subprocess(["python", "--version"], "Checking current Python", capture_output=True)
            ...
            return [
                {
                    ...
                    "url": f"https://reddit.com/r/example/post",
                    "type": "forum_post"
                }
            ]
    except Exception as e:
...
def run_llm_scoring(content_file: str, text: str, criteria: tuple) -> Dict[str, Any]:
    ...
    return {
        "overall_score": 4.2,
        "criteria_scores": {
            criterion: 4.0 for criterion in criteria
        },
        "reasoning": "Content contains relevant trading strategy information with technical analysis."
    }

def save_discovery_results(discoveries: List[Dict[str, Any]], topic: str, output_path: Path):
