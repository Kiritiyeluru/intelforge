...
class GitHubScraper(BaseScraper):
    ...
    def _initialize_github(self):
        ...
        try:
            ...
            access_token = (
                ...
                self.config['github']['access_token'] or
                self.config['api_keys']['github_token']
            )
            
            if not access_token or access_token == "your_github_token_here":
                self.logger.warning("GitHub access token not found. Using unauthenticated API (lower rate limits)")
                self.github = Github()
            else:
            ...
            self.logger.info(f"GitHub API initialized. Rate limit: {rate_limit.core.remaining}/{rate_limit.core.limit}")
    ...
    def search_repositories(self, query: str, limit: int = None) -> List[Dict]:
        ...
        self.logger.info(f"Found {len(scraped_repos)} relevant repositories for '{query}'")
...
def main():
    ...
    parser.add_argument('--limit', type=int, help="Limit number of repositories per query")
    parser.add_argument('--config', type=str, default="config/config.yaml", help="Path to config file")
    parser.add_argument('--rate-limit', action='store_true', help="Show rate limit status and exit")
        ...
        with GitHubScraper(config_path=args.config) as scraper:
            if args.rate_limit:
                ...
                if "error" in status:
                    ...
                    print(f"Core API: {status['core']['remaining']}/{status['core']['limit']}")
                    print(f"Search API: {status['search']['remaining']}/{status['search']['limit']}")
                return
