...
def create_timetable_model():
    ...
    for batch in BATCHES:
        ...
        for slot in WORKING_SLOTS:
            schedule[batch][slot] = model.NewIntVar(
                0, len(SUBJECTS) - 1, f"{batch}_slot_{slot}"
            )
        ...
        for slot in WORKING_SLOTS:
            ...
            for subject in SUBJECTS:
                for teacher in TEACHERS[subject]:
                    teacher_assigned[batch][slot][teacher] = model.NewBoolVar(
                        f"{batch}_{slot}_{teacher}"
                    )
    ...
    for batch in BATCHES:
        ...
        for subject_idx, subject in enumerate(SUBJECTS):
            ...
            for slot in WORKING_SLOTS:
                ...
                model.Add(schedule[batch][slot] == subject_idx).OnlyEnforceIf(
                    is_subject
                )
                model.Add(schedule[batch][slot] != subject_idx).OnlyEnforceIf(
                    is_subject.Not()
        ...
        for slot in WORKING_SLOTS:
            for subject_idx, subject in enumerate(SUBJECTS):
                ...
                teachers_for_subject = [
                    teacher_assigned[batch][slot][teacher]
                    for teacher in TEACHERS[subject]
                ]
                model.Add(sum(teachers_for_subject) == 1).OnlyEnforceIf(is_subject)
                model.Add(sum(teachers_for_subject) == 0).OnlyEnforceIf(
                    is_subject.Not()
                )
