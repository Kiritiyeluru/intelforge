...
FIXED_ASSIGNMENTS = {
    ...
    "9th_CLASS": {
        ...
        6: ("MATHS", "RAJESH", 0),  # Slot 6: 5:30-7:00 MATHS - RAJESH (2 sessions)
    },
}
...
def create_building_aware_model():
    ...
    for batch in BATCHES:
        if batch in FIXED_ASSIGNMENTS:
            for slot, (subject, teacher, session) in FIXED_ASSIGNMENTS[batch].items():
                batch_slots = (
                    NINTH_CLASS_WORKING_SLOTS if batch == "9th_CLASS" else WORKING_SLOTS
                )
    ...
    for batch in BATCHES:
        if batch == "9th_CLASS":
            ...
            for subject_idx, subject in enumerate(SUBJECTS):
                ...
                for slot in WORKING_SLOTS:
                    if (
                        slot in batch_schedule[batch]
                    ):  # Check if slot exists for this batch
                        is_subject = model.NewBoolVar(
                            f"{batch}_{slot}_{subject}_assigned"
                        )
                        model.Add(
                            batch_schedule[batch][slot]["subject"] == subject_idx
                        ).OnlyEnforceIf(is_subject)
    ...
    # Constraint 3: STRICT No-overlap teacher conflicts - global constraint
    ...
    for slot in all_possible_slots:
        ...
        # STRICT CONSTRAINT: Each teacher can teach AT MOST ONE class per slot (anywhere)
        ...
        if batch == "9th_CLASS":
            ...
            for subject_idx, subject in enumerate(SUBJECTS):
                ...
                for slot1, slot2 in ninth_class_adjacent_pairs:
                    if (
                        slot1 in batch_schedule[batch]
                        and slot2 in batch_schedule[batch]
                    ):
...
def solve_and_display_building_aware():
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
                    ...
                    if batch == "9th_CLASS":
                        ...
                        for slot_idx in range(len(TIME_SLOTS)):
                            if slot_idx == 0:  # Slot 1 - blocked for IOQM
                                ...
                                slot_idx in NINTH_CLASS_WORKING_SLOTS
                                and slot_idx in batch_schedule[batch]
                            ):
                                subject_idx = solver.Value(
                                    batch_schedule[batch][slot_idx]["subject"]
                                )
                                session_idx = solver.Value(
                                    batch_schedule[batch][slot_idx]["session"]
                                )
                        ...
                        for slot_idx in range(len(TIME_SLOTS)):
                            if slot_idx == LUNCH_SLOT:
                                ...
                                if (
                                    working_slot_idx is not None
                                    and slot_idx in WORKING_SLOTS
                                ):
                                    subject_idx = solver.Value(
                                        batch_schedule[batch][slot_idx]["subject"]
                                    )
        ...
        print("\n🚶 Travel Constraints:")
        print("   Building 1 ↔ Building 2: 0.55 km (no travel time)")
        print("   Building 3 ↔ Others: Far away (requires lunch/snack break)")
