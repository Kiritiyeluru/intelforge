...
def test_talib_performance():
    ...
    try:
        ...
        talib.SMA(prices, timeperiod=20)
        talib.EMA(prices, timeperiod=20)
        talib.RSI(prices, timeperiod=14)
        macd, macd_signal, macd_hist = talib.MACD(prices)
        bb_upper, bb_middle, bb_lower = talib.BBANDS(prices)
        slowk, slowd = talib.STOCH(prices, prices, prices)
        talib.ATR(prices, prices, prices, timeperiod=14)
        talib.ADX(prices, prices, prices, timeperiod=14)
    ...
    try:
        ...
        data = {
            ...
            "symbol": np.random.choice(
                ["AAPL", "GOOGL", "MSFT", "TSLA", "AMZN"], n_rows
            ),
        }
        ...
        df_pandas["volume_ma"] = (
            df_pandas.groupby("symbol")["volume"]
            .rolling(window=100)
            .mean()
            .reset_index(0, drop=True)
        )
        df_pandas.groupby(["symbol", df_pandas["timestamp"].dt.hour])["price"].agg(
            ["mean", "std", "count"]
        )
        ...
        df_polars.group_by(["symbol", pl.col("timestamp").dt.hour()]).agg(
            [
...
def test_financial_calculations():
    ...
    try:
        ...
        np.mean(portfolio_returns[portfolio_returns <= var_95])
...
def main():
    ...
    logger.info("=" * 60)
    success_rate = (
        (excellent_count + good_count) / total_tests if total_tests > 0 else 0
    )
    logger.info(
        f"📈 Overall Performance: {excellent_count + good_count}/{total_tests} tests successful ({success_rate:.1%})"
    )

    if excellent_count >= total_tests * 0.6:
        logger.info("🎉 EXCELLENT: Major performance improvements achieved!")
        logger.info(
            "🚀 Python 3.10 delivers significant speedups for financial computing"
        )
