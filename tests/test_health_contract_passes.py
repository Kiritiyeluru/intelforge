...
class TestHealthContract:
    ...
    def test_health_contract_schema(self):
        ...
        for field in required_fields:
            ...
            ), f"Required field '{field}' missing from status output"
    ...
    def test_storage_section_schema(self):
        ...
        for storage_name in expected_storage:
            assert (
                storage_name in storage
            ), f"Storage component '{storage_name}' missing"
    ...
    def test_json_schema_stability(self):
        ...
        for field, expected_type in core_schema.items():
            ...
            assert isinstance(
                status_data[field], expected_type
    ...
    def test_monitoring_integration(self):
        ...
        for storage_name, storage_info in status_data["storage"].items():
            if storage_info["exists"]:
                metrics[f"storage_{storage_name}_size_mb"] = storage_info[
                    "size_bytes"
                ] / (1024 * 1024)
                metrics[f"storage_{storage_name}_file_count"] = storage_info[
                    "file_count"
                ]
