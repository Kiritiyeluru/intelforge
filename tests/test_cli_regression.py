...
class TestCLIRegression:
    ...
    def test_crawl_command_missing_file(self):
        ...
        result = self.runner.invoke(
            app, ["crawl", str(non_existent_file), "--skip-validation"]
        )
    ...
    def test_crawl_command_validation_skip(self):
        ...
        with patch("scripts.semantic_crawler.main") as mock_crawler:
            ...
            result = self.runner.invoke(
                app, ["crawl", str(url_file), "--skip-validation", "--dry-run"]
            )
    ...
    def test_crawl_command_threshold_option(self):
        ...
        with patch("scripts.semantic_crawler.main") as mock_crawler:
            mock_crawler.return_value = None
            result = self.runner.invoke(
                app, ["crawl", str(url_file), "--threshold", "0.8", "--skip-validation"]
            )
    ...
    def test_validate_command_missing_config(self):
        ...
        with patch("scripts.cli.MODULES_AVAILABLE", True):
            result = self.runner.invoke(
                app, ["validate", "system", "--config", str(non_existent_config)]
            )
            assert result.exit_code == 1
    ...
    def test_validate_command_basic(self):
        ...
        with (
            patch("scripts.cli.MODULES_AVAILABLE", True),
    ...
    def test_status_command_basic(self):
        ...
        assert (
            "IntelForge System Status" in result.output
            or "Module Status" in result.output
        )
        assert (
            "Storage Status" in result.output or "Configuration Files" in result.output
        )
    ...
    def test_status_command_drift_flag(self):
        ...
        result = self.runner.invoke(
            app, ["status", "--json", "--drift", "--skip-validation"]
        )
        assert result.exit_code == 0
    ...
    def test_migrate_command_qdrant_to_chromadb(self):
        ...
        with patch(
            "scripts.vector_storage_migration.migrate_qdrant_to_chroma"
        ) as mock_migrate:
...
class TestCLIParameterValidation:
    ...
    def test_crawl_invalid_threshold(self):
        ...
        result = self.runner.invoke(
            app, ["crawl", str(url_file), "--threshold", "-0.1", "--skip-validation"]
        )
        ...
        # Test threshold > 1
        result = self.runner.invoke(
            app, ["crawl", str(url_file), "--threshold", "1.5", "--skip-validation"]
        )
        # Should not crash
    ...
    def test_status_command_combinations(self):
        ...
        result = self.runner.invoke(
            app, ["status", "--json", "--detailed", "--drift", "--skip-validation"]
        )
        assert result.exit_code == 0
    ...
    def test_migrate_command_verification_flags(self):
        ...
        with patch(
            ...
            if result.exit_code == 0:
                assert (
                    "Verifying migration" in result.output
                    or "Migration completed" in result.output
                )

            # Test with verification disabled
            result = self.runner.invoke(
                app, ["migrate", "qdrant", "chromadb", "--no-verify"]
            )
            if result.exit_code == 0:
...
class TestCLIErrorHandling:
    ...
    def test_crawl_command_module_failure(self):
        ...
        with (
            ...
            result = self.runner.invoke(
                app, ["crawl", str(url_file), "--skip-validation"]
            )
            assert result.exit_code == 1
    ...
    def test_docs_command_script_not_found(self):
        ...
        with patch("pathlib.Path.exists") as mock_exists:
            ...
            result = self.runner.invoke(
                app, ["docs", "create", "--type", "STS", "--title", "Test"]
            )
            assert result.exit_code == 1
    ...
    def test_docs_command_script_failure(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=1, stderr="Script execution failed"
            )
            result = self.runner.invoke(app, ["docs", "validate"])
            assert result.exit_code == 1
            assert "Validation failed" in result.output
    ...
    def test_migrate_command_import_error(self):
        ...
        with patch("builtins.__import__", side_effect=ImportError("No module")):
            ...
            assert result.exit_code == 1
            assert (
                "Migration modules not available" in result.output
...
class TestCLIBackwardsCompatibility:
    ...
    def test_output_format_consistency(self):
        ...
        for field in required_fields:
            ...
            ), f"Required field '{field}' missing from JSON output"
...
class TestCLIPerformance:
    ...
    def test_status_command_performance(self):
        ...
        assert (
            end_time - start_time
        ) < 30.0  # Should complete within 30 seconds (more lenient)
