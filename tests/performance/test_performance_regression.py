...
class PerformanceRegressionTester:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config_path = (
            config_path
            or self.project_root / "tests/performance/performance_baseline.json"
        )
    ...
    def _run_hyperfine_benchmark(
        self, command: str, baseline_config: Dict[str, Any]
        ...
        try:
            ...
            if not hyperfine_data.get("results"):
                print("❌ No results from hyperfine")
                return None
    ...
    def _generate_markdown_report(self, output_file: Path) -> None:
        ...
        for result in self.results:
            baseline_str = (
                f"{result.baseline_mean:.3f}s" if result.baseline_mean else "N/A"
            )
            regression_str = (
                f"{result.regression_factor:.2f}x"
                if result.regression_factor
                else "N/A"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="IntelForge Performance Regression Testing"
    )
    parser.add_argument("--config", help="Path to performance configuration file")
    parser.add_argument(
        "--cli-only", action="store_true", help="Run only CLI benchmarks"
    )
    parser.add_argument(
        "--core-only", action="store_true", help="Run only core operation benchmarks"
    )
