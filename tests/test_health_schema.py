...
class TestHealthSchema:
    ...
    def test_detailed_mode_schema_validation(self):
        ...
        try:
            ...
            if (
                validated_status.health_checks
                and validated_status.health_checks.error is None
                and validated_status.health_checks.detailed_results
            ):
    ...
    def test_schema_evolution_safety(self):
        ...
        extended_data = {
            ...
            "system_info": {"new_system_field": "should_be_ignored"},  # New field
        ...
        try:
            ...
            pytest.fail(
                f"Schema should accept additional fields for forward compatibility: {e}"
            )
    ...
    def test_schema_consistency_across_runs(self):
        ...
        for i in range(1, len(results)):
            ...
            assert set(results[0].keys()) == set(
                results[i].keys()
            ), "Top-level keys changed between runs"

            # Module structure should be consistent
            assert set(results[0]["modules"].keys()) == set(
                results[i]["modules"].keys()
            )

            # Storage structure should be consistent
            assert set(results[0]["storage"].keys()) == set(
                results[i]["storage"].keys()
            )

            # Config files structure should be consistent
            assert set(results[0]["config_files"].keys()) == set(
                results[i]["config_files"].keys()
            )
...
class TestSchemaBreakingChanges:
    ...
    def test_schema_version_compatibility(self):
        ...
        baseline_status = {
            ...
            "overall_status": "healthy",
            "modules": {"core_available": True, "status": "healthy"},
            "storage": {
                "qdrant": {
                    "exists": False,
                    "path": "/path/qdrant",
        ...
        try:
            ...
            pytest.fail(
                f"BREAKING CHANGE DETECTED: Baseline schema no longer validates: {e}"
            )
