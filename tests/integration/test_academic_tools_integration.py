...
try:
    from scripts.enhanced_research_gap_detector import EnhancedResearchGapDetector
...
@pytest.mark.skipif(not SEMANTIC_CRAWLER_AVAILABLE, 
                   reason="SemanticCrawler not available")
        ...
        try:
            ...
            with patch.dict(os.environ, {
                ...
                self.tester._log_test_result(
                    "semantic_crawler", "initialization", True,
                    {"attributes_checked": required_attrs}
                )
                
        except Exception as e:
            self.tester._log_test_result(
                "semantic_crawler", "initialization", False,
                {"error": str(e)}
            )
    ...
    def test_semantic_crawler_url_validation(self):
        ...
        try:
            with patch.dict(os.environ, {
                ...
                self.tester._log_test_result(
                    "semantic_crawler", "url_validation", True,
                    {
            ...
            self.tester._log_test_result(
                "semantic_crawler", "url_validation", False,
                {"error": str(e)}
            )
    ...
    def test_semantic_crawler_content_extraction(self, mock_get):
        ...
        try:
            ...
            with patch.dict(os.environ, {
                ...
                self.tester._log_test_result(
                    "semantic_crawler", "content_extraction", True,
                    {
                        "urls_tested": 3,
            ...
            self.tester._log_test_result(
                "semantic_crawler", "content_extraction", False,
                {"error": str(e)}
            )
            pytest.fail(f"Content extraction test failed: {e}")


@pytest.mark.skipif(not RESEARCH_GAP_DETECTOR_AVAILABLE, 
...
class TestResearchGapDetectorIntegration:
    ...
    def test_research_gap_detector_initialization(self):
        ...
        try:
            ...
            self.tester._log_test_result(
                "research_gap_detector", "initialization", True,
                {"methods_checked": required_methods}
            )
            
        except Exception as e:
            self.tester._log_test_result(
                "research_gap_detector", "initialization", False,
                {"error": str(e)}
            )
    ...
    def test_research_gap_detection(self):
        ...
        try:
            ...
            if hasattr(detector, 'detect_gaps'):
                ...
                assert isinstance(gaps, (list, dict)), "Gaps should be returned as list or dict"
                ...
                self.tester._log_test_result(
                    "research_gap_detector", "gap_detection", True,
                    {
                        "documents_analyzed": len(test_documents),
                        "gaps_detected": len(gaps) if isinstance(gaps, list) else len(gaps.keys())
                    }
                )
            else:
            ...
            self.tester._log_test_result(
                "research_gap_detector", "gap_detection", False,
                {"error": str(e)}
            )
...
class TestAcademicWorkflowIntegration:
        ...
        try:
            ...
            for test_query in self.tester.test_queries:
                ...
                if len(results['ids'][0]) >= min_results:
                    ...
                    domain_match = any(domain in ' '.join(found_domains) for domain in expected_domains)
            ...
            self.tester._log_test_result(
                "academic_workflow", "query_processing", workflow_success,
                {
            ...
            assert workflow_success, f"Academic workflow success rate too low: {success_rate:.2f}"
            
        except Exception as e:
            self.tester._log_test_result(
                "academic_workflow", "query_processing", False,
                {"error": str(e)}
            )
    ...
    def test_research_domain_classification(self):
        ...
        try:
            ...
            self.tester._log_test_result(
                "academic_workflow", "domain_classification", True,
                {
            ...
            assert classification_accuracy >= 0.5, f"Classification accuracy too low: {classification_accuracy:.2f}"
            
        except Exception as e:
            self.tester._log_test_result(
                "academic_workflow", "domain_classification", False,
                {"error": str(e)}
            )
...
class TestAcademicToolsHealthCheck:
    ...
    def test_academic_components_availability(self):
        ...
        core_available = components_status["sentence_transformers"] and components_status["chromadb"]
        
        self.tester._log_test_result(
            "academic_tools", "health_check", core_available,
            {
        ...
        assert core_available, "Core ML components (sentence-transformers + chromadb) must be available"
...
def generate_academic_integration_report(output_dir: str = "logs/academic_testing"):
    ...
    report_file = os.path.join(output_dir, f"academic_integration_report_{timestamp}.json")
...
if __name__ == "__main__":
    ...
    print(f"   ResearchGapDetector: {'✅' if RESEARCH_GAP_DETECTOR_AVAILABLE else '❌'}")
