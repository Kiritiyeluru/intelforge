...
class EndToEndWorkflowTester:
    ...
    def __init__(self):
        ...
        self.e2e_db_path = (
            ...
            / "reports"
            / "end_to_end"
            / f"e2e_session_{self.timestamp}.db"
        )
        ...
        try:
            ...
            for i, article in enumerate(loaded_articles):
                ...
                if valid:
                    ...
                    if not isinstance(article["tags"], list):
                        ...
                        validation_errors.append(
                            f"Article {i + 1}: Tags must be a list"
                        )
                ...
                if valid:
                    ...
                    validation_errors.append(
                        f"Article {i + 1}: Missing fields: {missing_fields}"
                    )
            ...
            integrity_score = (
                len(transformed_articles) / len(loaded_articles)
                if loaded_articles
                else 0
            )

            self.record_data_flow(
                "data_ingestion", "json", "processed_json", integrity_score, 1.0, "pass"
            )
        ...
        try:
            ...
            for article in articles:
                ...
                analysis = {
                    ...
                    / 100,  # Word count based complexity
                    "technical_terms": len(
                        [
                            word
                            for word in content.lower().split()
                            if word
                            in ["trading", "analysis", "market", "risk", "portfolio"]
                        ]
                    ),
            ...
            for article in analyzed_articles:
                ...
                confidence = category_scores[primary_category] / len(
            ...
            for article in categorized_articles:
                ...
                quality_components = {
                    "content_length": min(
                        1.0, article["word_count"] / 200
                    ),  # Prefer 200+ words
                    "technical_relevance": analysis["technical_terms"]
                    / 10,  # More technical terms = higher quality
                ...
                overall_quality = sum(quality_components.values()) / len(
                    quality_components
                )
            ...
            high_quality_articles = [
                a for a in scored_articles if a["quality"]["grade"] == "high"
            ]
            accuracy = (
                len(high_quality_articles) / len(scored_articles)
                if scored_articles
                else 0
            )
            ...
            return {
                ...
                "average_quality_score": sum(
                    a["quality"]["overall_score"] for a in scored_articles
                )
                / len(scored_articles),
        ...
        try:
            ...
            for query in search_queries:
                ...
                for chunk in embedded_chunks[:10]:  # Top 10 results
                    ...
                    similarity = len(query_words & content_words) / len(
                ...
                search_results.append(
                    {
                        ...
                        "avg_similarity": sum(
                            r["similarity_score"] for r in query_results[:5]
                        )
                        / 5,
                    }
                )
            ...
            ai_results = {
                ...
                "metadata": {
                    ...
                    "average_search_similarity": sum(
                        r["avg_similarity"] for r in search_results
                    )
                    / len(search_results),
                },
            }
    ...
    def test_output_generation_workflow(self) -> Dict:
        ...
        try:
            ...
            summary_report = {
                "executive_summary": {
                    "total_documents_processed": len(
                        set(chunk["article_id"] for chunk in chunks)
                    ),
                ...
                "content_analysis": {
                    "categories_identified": list(
                        set(chunk["category"] for chunk in chunks)
                    ),
                    "average_chunk_length": sum(
                ...
                "search_analytics": {
                    ...
                    "average_results_per_query": sum(
                        len(r["results"]) for r in search_results
                    )
                    / len(search_results),
            ...
            for chunk in chunks:
                ...
                if (
                    category
                    in summary_report["content_analysis"]["content_distribution"]
                ):
                    summary_report["content_analysis"]["content_distribution"][
                        category
                    ] += 1
                else:
                    summary_report["content_analysis"]["content_distribution"][
...
"""
            ...
            for category, count in summary_report["content_analysis"][
                "content_distribution"
            ].items():
...
"""
            ...
            for result in search_results:
                ...
                markdown_content += (
                    f"  - **Average Similarity**: {result['avg_similarity']:.3f}\n"
                )
...
"""
            ...
            phase_duration = time.time() - phase_start
            self.record_workflow_test(
                workflow_name, "markdown_generation", "pass", phase_duration, 1, 1
            )
            ...
            phase_duration = time.time() - phase_start
            self.record_workflow_test(
                workflow_name, "json_export", "pass", phase_duration, 1, 1
            )

            workflow_duration = time.time() - workflow_start

            # Calculate output quality
            [summary_report, markdown_file.stat().st_size, json_file.stat().st_size]

            self.record_data_flow(
                "output_generation", "vector_embeddings", "reports", 1.0, 1.0, "pass"
            )
        ...
        try:
            ...
            if file_integrity["ai_pipeline_results.json"]["exists"]:
                with open(self.test_output_dir / "ai_pipeline_results.json", "r") as f:
                    ai_results = json.load(f)
                    consistency_checks["total_chunks"] = len(
                        ai_results.get("chunks", [])
                    )
    ...
    def generate_e2e_report(self) -> str:
        ...
        cursor.execute(
            "SELECT AVG(duration_seconds) FROM workflow_tests WHERE duration_seconds > 0"
        )
        ...
        report_path = (
            ...
            / "reports"
            / "end_to_end"
            / f"e2e_report_{self.timestamp}.json"
        )
        ...
        md_report_path = (
            self.test_dir / "reports" / "end_to_end" / f"e2e_report_{self.timestamp}.md"
        )
        self.create_e2e_markdown_report(
            md_report_path, passed_tests, failed_tests, avg_duration
        )

        return str(md_report_path)

    def create_e2e_markdown_report(
        self, path: Path, passed: int, failed: int, avg_duration: float
    ):
        ...
        for workflow_name, results in self.results["workflows"].items():
            ...
            if "error" in results:
                ...
                if workflow_name == "data_ingestion":
                    ...
                    content += (
                        f"**Chunks Generated**: {results.get('total_chunks', 0)}  \n"
                    )
        ...
        if "integrity_checks" in self.results:
            ...
            if "integrity_results" in integrity:
                ...
                for filename, info in (
                    integrity["integrity_results"].get("file_integrity", {}).items()
                ):
...
def main():
    parser = argparse.ArgumentParser(
        description="IntelForge End-to-End Workflow Tester"
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    parser.add_argument(
        "--keep-data", action="store_true", help="Keep test data after completion"
    )
    ...
    successful_workflows = sum(
        1 for w in workflows.values() if w.get("status", "").startswith("✅")
    )
    ...
    print(
        f"📊 **Workflows**: {successful_workflows}/{total_workflows} successful ({success_rate:.1f}%)"
    )
    print(f"🔍 **Data Integrity**: {integrity_score:.2f}")
    print(
        f"📋 **Pipeline Status**: {'HEALTHY' if success_rate >= 80 and integrity_score >= 0.8 else 'NEEDS ATTENTION'}"
    )
