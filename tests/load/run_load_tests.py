...
class LoadTestOrchestrator:
    ...
    def run_single_test(self, persona: str, verbose: bool = False) -> LoadTestResult:
        ...
        try:
            ...
            cmd = [
                ...
                "run",
                "--out",
                f"json={self.results_dir}/k6_{persona}_metrics.json",
                str(test_file),
            ]
    ...
    def generate_report(self, results: List[LoadTestResult], total_duration: float):
        ...
        report = {
            ...
            "detailed_metrics": {r.persona: r.metrics for r in results if r.metrics},
        }

        # Save JSON report
        report_file = (
            self.results_dir / f"load_test_report_{timestamp.replace(':', '-')}.json"
        )
...
def main():
    ...
    parser.add_argument(
        "--persona",
        choices=["researcher", "trader", "developer"],
