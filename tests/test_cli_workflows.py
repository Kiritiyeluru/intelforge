...
"""
        ...
        self.config_file.write_text(
            json.dumps(
                {
                    "validation_settings": {"threshold": 0.75, "timeout": 30},
                    "output_format": "json",
                },
                indent=2,
            )
    ...
    def test_crawl_workflow_dry_run(self):
        ...
        with (
            ...
            result = self.runner.invoke(
                app, ["crawl", str(self.url_file), "--dry-run", "--skip-validation"]
            )
    ...
    def test_error_handling_workflow(self):
        ...
        result = self.runner.invoke(
            app, ["crawl", str(missing_file), "--skip-validation"]
        )
        assert result.exit_code == 1
...
class TestWorkflowIntegration:
    ...
    def test_data_pipeline_workflow(self):
        ...
        with (
            ...
            # 2. Run crawling
            result = self.runner.invoke(
                app, ["crawl", str(url_file), "--dry-run", "--skip-validation"]
            )
    ...
    def test_diagnostic_workflow(self):
        ...
        result = self.runner.invoke(app, ["status", "--detailed", "--drift"])
        assert result.exit_code == 0
...
class TestWorkflowPerformance:
    ...
    def test_workflow_consistency(self):
        ...
        for i in range(1, len(results)):
            ...
            assert results[i]["overall_status"] in [
                "healthy",
                "degraded",
                "critical",
                "unknown",
            ...
            assert (
                results[0]["modules"]["core_available"]
                == results[i]["modules"]["core_available"]
            )
...
class TestWorkflowErrorRecovery:
    ...
    def test_partial_failure_recovery(self):
        ...
        result = self.runner.invoke(
            app, ["crawl", str(bad_url_file), "--skip-validation"]
        )
