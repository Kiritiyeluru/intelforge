...
class PerformanceMetrics:
    ...
    def __post_init__(self):
        self.speedup_factor = self.standard_time / self.rust_time if self.rust_time > 0 else 0.0
...
class RustEnhancedPipeline:
        ...
        try:
            ...
            self.qdrant_client.create_collection(
                ...
                vectors_config=VectorParams(
                    size=self.config.get('vector_dim', 384),
    ...
    def benchmark_tokenization(self, texts: List[str]) -> PerformanceMetrics:
        ...
        for text in texts:
            try:
                tokens = self.sentence_model.tokenizer.encode(text)
            except Exception as e:
                self.logger.warning(f"Standard tokenization failed: {e}")
        standard_time = time.time() - start_time
        
        metrics = PerformanceMetrics("tokenization", rust_time, standard_time, 0, len(texts))
        self.metrics.append(metrics)
        
        self.logger.info(f"Tokenization benchmark: {metrics.speedup_factor:.2f}x speedup")
        return metrics
    ...
    def benchmark_vector_search(self, texts: List[str]) -> PerformanceMetrics:
        ...
        metrics = PerformanceMetrics("vector_search", qdrant_time, faiss_time, 0, len(texts))
        self.metrics.append(metrics)
        
        self.logger.info(f"Vector search benchmark: {metrics.speedup_factor:.2f}x speedup")
        return metrics
    ...
    def benchmark_data_processing(self, data: List[Dict]) -> PerformanceMetrics:
        ...
        metrics = PerformanceMetrics("data_processing", polars_time, pandas_time, 0, len(data))
        self.metrics.append(metrics)
        
        self.logger.info(f"Data processing benchmark: {metrics.speedup_factor:.2f}x speedup")
        return metrics
    ...
    def benchmark_database_operations(self, data: List[Dict]) -> PerformanceMetrics:
        ...
        try:
            ...
            self.duckdb_conn.execute("CREATE TABLE IF NOT EXISTS test_data AS SELECT * FROM df")
        ...
        metrics = PerformanceMetrics("database_ops", duckdb_time, pandas_time, 0, len(data))
        self.metrics.append(metrics)
        
        self.logger.info(f"Database operations benchmark: {metrics.speedup_factor:.2f}x speedup")
        return metrics
        ...
        data = [
            {
                ...
                "category": np.random.choice(["finance", "trading", "analysis", "research"]),
    ...
    async def run_comprehensive_benchmark(self, data_size: int = 1000):
        ...
        self.logger.info(f"Starting comprehensive benchmark with {data_size} records...")
        ...
        tokenization_metrics = self.benchmark_tokenization(texts[:500])
        
        # 2. Vector search benchmark
        vector_search_metrics = self.benchmark_vector_search(texts[:150])
        
        # 3. Data processing benchmark
        data_processing_metrics = self.benchmark_data_processing(data[:5000])
        
        # 4. Database operations benchmark
        database_metrics = self.benchmark_database_operations(data[:5000])
    ...
    def generate_performance_report(self):
        ...
        total_speedup = sum(m.speedup_factor for m in self.metrics if m.speedup_factor > 0)
        ...
        for metric in self.metrics:
            self.logger.info(f"{metric.operation}: {metric.speedup_factor:.2f}x speedup")
        self.logger.info("==========================================")
...
async def main():
    ...
    try:
        ...
        print(f"✅ Average speedup: {sum(m.speedup_factor for m in metrics) / len(metrics):.2f}x")
        print("✅ Report saved to vault/performance_reports/rust_enhanced_benchmark.json")
