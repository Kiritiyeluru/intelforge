...
class ProductionReadinessAssessor:
    ...
    def __init__(self):
        ...
        # Assessment database
        self.assessment_db_path = self.test_dir / "reports" / "production_readiness" / f"assessment_{self.timestamp}.db"
        ...
        self.assessment_categories = {
            "infrastructure": {"weight": 0.25, "description": "System infrastructure and dependencies"},
            "security": {"weight": 0.20, "description": "Security posture and vulnerability assessment"},
            "performance": {"weight": 0.20, "description": "Performance benchmarks and scalability"},
            "reliability": {"weight": 0.15, "description": "Error handling and fault tolerance"},
            "monitoring": {"weight": 0.10, "description": "Observability and monitoring capabilities"},
            "documentation": {"weight": 0.10, "description": "Documentation completeness and quality"}
        }
    ...
    def record_assessment_result(self, category: str, test_name: str, score: float, 
                                max_score: float, status: str, details: str = "", 
                                recommendations: str = ""):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (datetime.datetime.now().isoformat(), category, test_name, score, max_score,
              status, details, recommendations))
        
        conn.commit()
        conn.close()
    
    def record_blocking_issue(self, issue_type: str, severity: str, description: str,
                            resolution: str, category: str):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (datetime.datetime.now().isoformat(), issue_type, severity, description,
              resolution, category))
    ...
    def assess_infrastructure_readiness(self) -> Dict:
        ...
        critical_dirs = [
            "scrapers", "scripts", "vault", "vault/notes", "vault/logs", 
            "config", "session_docs"
        ]
        
        directory_score = 0
        max_directory_score = len(critical_dirs) * 10
        
        for directory in critical_dirs:
            dir_path = self.project_root / directory
            if dir_path.exists() and dir_path.is_dir():
                directory_score += 10
                self.record_assessment_result("infrastructure", f"directory_{directory}", 
                                            10, 10, "pass", f"Directory exists: {dir_path}")
            else:
                self.record_blocking_issue("missing_directory", "high", 
                                         f"Critical directory missing: {directory}",
                                         "Create missing directory structure", "infrastructure")
                self.record_assessment_result("infrastructure", f"directory_{directory}",
                                            0, 10, "fail", f"Directory missing: {dir_path}")
        ...
        config_score = 0
        max_config_score = len(config_files) * 10
        ...
        for config_file in config_files:
            ...
            if config_path.exists():
                try:
                    ...
                    config_score += 10
                    self.record_assessment_result("infrastructure", f"config_{config_file.replace('/', '_')}", 
                                                10, 10, "pass", f"Valid configuration: {config_path}")
                except Exception as e:
                    config_score += 5  # Partial credit for existence
                    self.record_assessment_result("infrastructure", f"config_{config_file.replace('/', '_')}", 
                                                5, 10, "partial", f"Configuration exists but invalid: {e}")
            else:
                if "settings.json" not in config_file:  # Claude settings is optional
                    self.record_blocking_issue("missing_config", "medium",
                                             f"Configuration file missing: {config_file}",
                                             "Create or restore configuration file", "infrastructure")
                self.record_assessment_result("infrastructure", f"config_{config_file.replace('/', '_')}", 
                                            0, 10, "fail", f"Configuration missing: {config_path}")
        ...
        module_score = 0
        max_module_score = len(core_modules) * 5
        ...
        for module in core_modules:
            ...
            if module_path.exists():
                try:
                    ...
                    module_score += 5
                    self.record_assessment_result("infrastructure", f"module_{module.replace('/', '_')}", 
                                                5, 5, "pass", f"Module syntax valid: {module_path}")
                except SyntaxError as e:
                    self.record_blocking_issue("syntax_error", "high",
                                             f"Syntax error in {module}: {e}",
                                             "Fix syntax errors in core module", "infrastructure")
                    self.record_assessment_result("infrastructure", f"module_{module.replace('/', '_')}", 
                                                0, 5, "fail", f"Syntax error: {e}")
            else:
                self.record_blocking_issue("missing_module", "high",
                                         f"Core module missing: {module}",
                                         "Restore or recreate missing core module", "infrastructure")
                self.record_assessment_result("infrastructure", f"module_{module.replace('/', '_')}", 
                                            0, 5, "fail", f"Module missing: {module_path}")
        ...
        try:
            ...
            if sys.version_info >= (3, 10):
                python_score += 10
                self.record_assessment_result("infrastructure", "python_version", 10, 10, "pass",
                                            f"Python version {sys.version} is supported")
            else:
                self.record_blocking_issue("python_version", "high",
                                         f"Python version {sys.version} is too old",
                                         "Upgrade to Python 3.10 or higher", "infrastructure")
                self.record_assessment_result("infrastructure", "python_version", 0, 10, "fail",
            ...
            self.record_assessment_result("infrastructure", "python_packages", package_score, 10, 
            ...
            self.record_assessment_result("infrastructure", "python_environment", 0, 20, "error",
                                        f"Python environment check failed: {e}")
        
        infrastructure_score += python_score
        
        # Normalize score
        infrastructure_percentage = (infrastructure_score / max_infrastructure_score) * 100
        
        return {
            "score": infrastructure_percentage,
            "max_score": 100,
            "status": "excellent" if infrastructure_percentage >= 90 else "good" if infrastructure_percentage >= 75 else "needs_attention",
            "details": {
    ...
    def assess_security_readiness(self) -> Dict:
        ...
        exposure_score = 50  # Start with full score, deduct for issues
        ...
        for config_file in config_files:
            if config_file.name.startswith('.') or 'test' in str(config_file):
                continue
            ...
            try:
                ...
                for pattern in sensitive_patterns:
                    if pattern in content and not content.count(pattern) == content.count(f"{pattern}_file"):
                        exposure_score -= 10
                        self.record_blocking_issue("hardcoded_secrets", "critical",
                                                 f"Potential hardcoded secret in {config_file}",
                                                 "Move secrets to environment variables or secure vault", "security")
                        break
                        
            except Exception:
                pass
        
        security_score += max(0, exposure_score)
        self.record_assessment_result("security", "secret_exposure_check", max(0, exposure_score), 50,
                                    "pass" if exposure_score == 50 else "fail",
                                    f"Secret exposure score: {max(0, exposure_score)}/50")
            ...
            if full_path.exists():
                try:
                    ...
                    if stat_info.st_mode & 0o044:  # World or group readable
                        permission_score -= 5
                        self.record_assessment_result("security", f"permissions_{file_path.replace('/', '_')}", 
                                                    0, 5, "warning", f"File may be too permissive: {full_path}")
                    else:
                        self.record_assessment_result("security", f"permissions_{file_path.replace('/', '_')}", 
                                                    5, 5, "pass", f"File permissions acceptable: {full_path}")
                except Exception as e:
                    self.record_assessment_result("security", f"permissions_{file_path.replace('/', '_')}", 
                                                0, 5, "error", f"Cannot check permissions: {e}")
        
        security_score += permission_score
        
        # Check for security best practices in code
        code_security_score = 25
            ...
            try:
                ...
                for pattern in dangerous_patterns:
                    if pattern in content:
                        security_issues += 1
                        self.record_assessment_result("security", f"dangerous_code_{code_file.name}", 
                                                    0, 2, "warning", 
                                                    f"Potentially dangerous pattern {pattern} in {code_file}")
                        break
        ...
        code_security_score -= min(25, security_issues * 3)
        security_score += max(0, code_security_score)
        
        self.record_assessment_result("security", "code_security_analysis", max(0, code_security_score), 25,
                                    "pass" if security_issues == 0 else "warning",
        ...
        return {
            ...
            "max_score": 100,
            "status": "excellent" if security_percentage >= 90 else "good" if security_percentage >= 75 else "needs_attention",
            "details": {
                "exposure_score": max(0, exposure_score),
                "permission_score": permission_score,
                "code_security_score": max(0, code_security_score),
    ...
    def assess_performance_readiness(self) -> Dict:
        ...
        if baseline_file.exists():
            try:
                ...
                if baseline_data:
                    baseline_score = 25
                    self.record_assessment_result("performance", "baseline_metrics", 25, 25, "pass",
                                                f"Performance baseline established with {len(baseline_data)} metrics")
                else:
                    baseline_score = 10
                    self.record_assessment_result("performance", "baseline_metrics", 10, 25, "partial",
                                                "Performance baseline file exists but is empty")
                    
            except Exception as e:
                self.record_assessment_result("performance", "baseline_metrics", 0, 25, "error",
                                            f"Cannot read performance baseline: {e}")
        else:
            self.record_assessment_result("performance", "baseline_metrics", 0, 25, "missing",
        ...
        performance_reports = list((self.test_dir / "reports" / "performance_regression").glob("performance_report_*.md"))
        ...
        if performance_reports:
            ...
            try:
                ...
                if "✅ HEALTHY" in content:
                    ...
                    self.record_assessment_result("performance", "recent_test_results", 25, 25, "pass",
                                                f"Recent performance tests show healthy status")
                elif "⚠️" in content:
                    recent_performance_score = 15
                    self.record_assessment_result("performance", "recent_test_results", 15, 25, "warning",
                                                "Recent performance tests show warnings")
                else:
                    recent_performance_score = 5
                    self.record_assessment_result("performance", "recent_test_results", 5, 25, "poor",
                                                "Recent performance tests show issues")
                    
            except Exception as e:
                self.record_assessment_result("performance", "recent_test_results", 0, 25, "error",
                                            f"Cannot read performance report: {e}")
        else:
            self.record_assessment_result("performance", "recent_test_results", 0, 25, "missing",
        ...
        integration_reports = list((self.test_dir / "reports" / "integration_tests").glob("integration_report_*.md"))
        integration_score = 0
        
        if integration_reports:
            latest_integration = max(integration_reports, key=lambda p: p.stat().st_mtime)
            ...
            try:
                ...
                if "Success Rate" in content:
                    ...
                    if match:
                        ...
                        status = "excellent" if success_rate >= 95 else "good" if success_rate >= 85 else "needs_attention"
                        self.record_assessment_result("performance", "integration_success_rate", 
                                                    integration_score, 25, status,
                                                    f"Integration test success rate: {success_rate}%")
                        
            except Exception as e:
                self.record_assessment_result("performance", "integration_success_rate", 0, 25, "error",
                                            f"Cannot analyze integration results: {e}")
        else:
            self.record_assessment_result("performance", "integration_success_rate", 0, 25, "missing",
                                        "No integration test results found")
        
        performance_score += integration_score
        
        # Check end-to-end test results
        e2e_reports = list((self.test_dir / "reports" / "end_to_end").glob("e2e_report_*.md"))
        e2e_score = 0
        ...
        if e2e_reports:
            ...
            try:
                ...
                if "✅ EXCELLENT" in content:
                    e2e_score = 25
                    self.record_assessment_result("performance", "e2e_test_results", 25, 25, "excellent",
                                                "End-to-end tests show excellent performance")
                elif "✅ GOOD" in content:
                    e2e_score = 20
                    self.record_assessment_result("performance", "e2e_test_results", 20, 25, "good",
                                                "End-to-end tests show good performance")
                elif "⚠️" in content:
                    e2e_score = 10
                    self.record_assessment_result("performance", "e2e_test_results", 10, 25, "warning",
                                                "End-to-end tests show performance warnings")
                else:
                    e2e_score = 5
                    self.record_assessment_result("performance", "e2e_test_results", 5, 25, "poor",
                                                "End-to-end tests show performance issues")
                    
            except Exception as e:
                self.record_assessment_result("performance", "e2e_test_results", 0, 25, "error",
                                            f"Cannot analyze e2e results: {e}")
        else:
            self.record_assessment_result("performance", "e2e_test_results", 0, 25, "missing",
        ...
        return {
            ...
            "max_score": 100,
            "status": "excellent" if performance_percentage >= 90 else "good" if performance_percentage >= 75 else "needs_attention",
            "details": {
    ...
    def assess_reliability_readiness(self) -> Dict:
        ...
        for module in core_modules:
            ...
            if module_path.exists():
                try:
                    ...
                    pattern_count = sum(content.count(pattern) for pattern in error_patterns)
                    
                    # Score based on error handling density
                    lines = content.count('\n')
                    if lines > 0:
                        error_density = pattern_count / lines
                        module_score = min(12.5, error_density * 1000)  # Max 12.5 points per module
                        error_handling_score += module_score
                        
                        status = "excellent" if error_density >= 0.02 else "good" if error_density >= 0.01 else "needs_improvement"
                        self.record_assessment_result("reliability", f"error_handling_{module.replace('/', '_')}", 
                                                    module_score, 12.5, status,
                                                    f"Error handling density: {error_density:.3f}")
                    
                except Exception as e:
                    self.record_assessment_result("reliability", f"error_handling_{module.replace('/', '_')}", 
                                                0, 12.5, "error", f"Cannot analyze module: {e}")
            else:
                self.record_assessment_result("reliability", f"error_handling_{module.replace('/', '_')}", 
                                            0, 12.5, "missing", f"Module not found: {module}")
        ...
        for code_file in code_files:
            ...
            try:
                ...
                if any(pattern in content for pattern in ["import logging", "logger", "log."]):
                    logging_modules += 1
        ...
        self.record_assessment_result("reliability", "logging_implementation", logging_score, max_logging_score,
                                    "good" if logging_modules >= 3 else "partial" if logging_modules > 0 else "missing",
        ...
        fault_tolerance_patterns = ["retry", "timeout", "circuit", "fallback", "backoff"]
        ...
        fault_tolerance_score = min(max_fault_tolerance_score, fault_tolerance_found * 3)
        
        self.record_assessment_result("reliability", "fault_tolerance", fault_tolerance_score, max_fault_tolerance_score,
                                    "excellent" if fault_tolerance_found >= 5 else "good" if fault_tolerance_found >= 3 else "needs_improvement",
        ...
        return {
            ...
            "max_score": 100,
            "status": "excellent" if reliability_percentage >= 90 else "good" if reliability_percentage >= 75 else "needs_attention",
            "details": {
    ...
    def assess_monitoring_readiness(self) -> Dict:
        ...
        if vault_logs.exists() and vault_logs.is_dir():
            ...
            if log_files:
                ...
                self.record_assessment_result("monitoring", "log_infrastructure", 30, 30, "pass",
                                            f"Log directory exists with {len(log_files)} log files")
            else:
                log_infrastructure_score = 15
                self.record_assessment_result("monitoring", "log_infrastructure", 15, 30, "partial",
                                            "Log directory exists but no log files found")
        else:
            self.record_assessment_result("monitoring", "log_infrastructure", 0, 30, "missing",
        ...
        if reports_dir.exists():
            ...
            self.record_assessment_result("monitoring", "reporting_infrastructure", reports_score, 40, 
                                        "excellent" if len(report_types) >= 4 else "good" if len(report_types) >= 2 else "basic",
                                        f"Report categories available: {len(report_types)}")
        else:
            self.record_assessment_result("monitoring", "reporting_infrastructure", 0, 40, "missing",
        ...
        self.record_assessment_result("monitoring", "dashboard_capabilities", dashboard_score, 30,
        ...
        return {
            ...
            "max_score": 100,
            "status": "excellent" if monitoring_percentage >= 90 else "good" if monitoring_percentage >= 75 else "needs_attention",
            "details": {
    ...
    def assess_documentation_readiness(self) -> Dict:
        ...
        essential_docs = [
            "README.md",
            "CLAUDE.md",
            "session_docs/PROJECT_STATUS.md"
        ]
        
        essential_docs_score = 0
        max_essential_score = 40
            ...
            if doc_path.exists():
                try:
                    ...
                    if len(content) > 500:  # Substantial content
                        ...
                        self.record_assessment_result("documentation", f"essential_{doc_file.replace('/', '_')}", 
                                                    13.33, 13.33, "pass", 
                    ...
                    self.record_assessment_result("documentation", f"essential_{doc_file.replace('/', '_')}", 
                                                0, 13.33, "error", f"Cannot read documentation: {e}")
            else:
                if "README.md" in doc_file:
                    self.record_blocking_issue("missing_readme", "medium",
                                             "README.md file is missing",
                                             "Create comprehensive README with project overview and setup instructions",
                                             "documentation")
                
                self.record_assessment_result("documentation", f"essential_{doc_file.replace('/', '_')}", 
                                            0, 13.33, "missing", f"Essential documentation missing: {doc_file}")
        ...
        if session_docs_dir.exists():
            ...
            self.record_assessment_result("documentation", "session_documentation", session_score, max_session_score,
                                        "excellent" if len(session_files) >= 6 else "good" if len(session_files) >= 3 else "basic",
                                        f"Session documentation files: {len(session_files)}")
        else:
            self.record_assessment_result("documentation", "session_documentation", 0, max_session_score, "missing",
        ...
        self.record_assessment_result("documentation", "technical_documentation", technical_docs_score, max_technical_score,
                                    "excellent" if tech_doc_count >= 15 else "good" if tech_doc_count >= 8 else "basic",
        ...
        return {
            ...
            "max_score": 100,
            "status": "excellent" if documentation_percentage >= 90 else "good" if documentation_percentage >= 75 else "needs_attention",
            "details": {
    ...
    def generate_readiness_report(self) -> str:
        ...
        report_path = self.test_dir / "reports" / "production_readiness" / f"readiness_report_{self.timestamp}.json"
        ...
        md_report_path = self.test_dir / "reports" / "production_readiness" / f"readiness_report_{self.timestamp}.md"
...
"""
        ...
        for category, results in overall["categories"].items():
            ...
            status_emoji = "✅" if results["score"] >= 90 else "⚠️" if results["score"] >= 75 else "❌"
            ...
            if "details" in results:
                ...
                for metric, value in results["details"].items():
                    if isinstance(value, (int, float)):
                        content += f"- **{metric.replace('_', ' ').title()}**: {value:.1f}\n"
                    else:
...
def main():
    parser = argparse.ArgumentParser(description="IntelForge Production Readiness Assessor")
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    parser.add_argument("--category", choices=["infrastructure", "security", "performance", "reliability", "monitoring", "documentation"],
    ...
    # Run assessments
    categories_to_assess = [args.category] if args.category else list(assessor.assessment_categories.keys())
    
    for category in categories_to_assess:
        print(f"\n{'='*80}")
        
        if category == "infrastructure":
            assessor.assessment_results["categories"][category] = assessor.assess_infrastructure_readiness()
        elif category == "security":
            assessor.assessment_results["categories"][category] = assessor.assess_security_readiness()
        elif category == "performance":
            assessor.assessment_results["categories"][category] = assessor.assess_performance_readiness()
        elif category == "reliability":
            assessor.assessment_results["categories"][category] = assessor.assess_reliability_readiness()
        elif category == "monitoring":
            assessor.assessment_results["categories"][category] = assessor.assess_monitoring_readiness()
        elif category == "documentation":
            assessor.assessment_results["categories"][category] = assessor.assess_documentation_readiness()
    ...
    print(f"🚀 **Readiness Level**: {overall_results['readiness_level'].replace('_', ' ').title()}")
