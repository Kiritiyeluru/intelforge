...
class IntelForgeIntegrationTester:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.test_db_path = self.test_dir / "reports" / "integration_tests" / f"test_session_{self.timestamp}.db"
    ...
    def record_test_result(self, category: str, name: str, status: str, 
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (datetime.datetime.now().isoformat(), category, name, status, duration, output, error))
        ...
        for module_name, file_name in core_modules:
            ...
            try:
                ...
                if module_path.exists():
                    ...
                    self.record_test_result("core_imports", file_name, "pass", 
                    ...
                    self.record_test_result("core_imports", file_name, "missing", 
                                          (datetime.datetime.now() - start_time).total_seconds(),
                                          error="File not found")
                ...
                self.record_test_result("core_imports", file_name, "syntax_error", 
                                      (datetime.datetime.now() - start_time).total_seconds(),
                                      error=str(e))
                ...
                self.record_test_result("core_imports", file_name, "error", 
                                      (datetime.datetime.now() - start_time).total_seconds(),
                                      error=str(e))
            finally:
    ...
    def test_configuration_loading(self) -> Dict:
        ...
        try:
            ...
            self.record_test_result("configuration", "config_loading", "pass",
                                  (datetime.datetime.now() - start_time).total_seconds())
                                  
        except Exception as e:
            config_results["error"] = str(e)
            self.record_test_result("configuration", "config_loading", "error",
                                  (datetime.datetime.now() - start_time).total_seconds(),
                                  error=str(e))
    ...
    def test_database_connections(self) -> Dict:
        ...
        try:
            ...
            self.record_test_result("database", "sqlite_basic", "pass",
                                  (datetime.datetime.now() - start_time).total_seconds())
            ...
            self.record_test_result("database", "sqlite_basic", "error",
                                  (datetime.datetime.now() - start_time).total_seconds(),
                                  error=str(e))
        ...
        try:
            ...
            if vector_db_available:
                ...
                self.record_test_result("database", f"vector_db_{vector_db_type}", "pass",
                                      (datetime.datetime.now() - start_time).total_seconds())
            else:
                db_results["vector_database"] = {
                ...
                self.record_test_result("database", "vector_db", "not_available",
                                      (datetime.datetime.now() - start_time).total_seconds())
            ...
            self.record_test_result("database", "vector_db", "error",
                                  (datetime.datetime.now() - start_time).total_seconds(),
                                  error=str(e))
        ...
        try:
            ...
            if scraping_base_path.exists():
                ...
                if result.returncode == 0:
                    ...
                    self.record_test_result("scrapers", "scraping_base_syntax", "pass",
                                          (datetime.datetime.now() - start_time).total_seconds())
                else:
                    scraper_results["scraping_base"] = {
                    ...
                    self.record_test_result("scrapers", "scraping_base_syntax", "compilation_error",
                                          (datetime.datetime.now() - start_time).total_seconds(),
                                          error=result.stderr)
            else:
                scraper_results["scraping_base"] = {
                ...
                self.record_test_result("scrapers", "scraping_base_syntax", "missing",
                                      (datetime.datetime.now() - start_time).total_seconds())
            ...
            self.record_test_result("scrapers", "scraping_base_syntax", "error",
                                  (datetime.datetime.now() - start_time).total_seconds(),
                                  error=str(e))
            ...
            try:
                ...
                if scraper_path.exists():
                    ...
                    if result.returncode == 0:
                        ...
                        self.record_test_result("scrapers", f"{scraper_file}_syntax", "pass",
                                              (datetime.datetime.now() - start_time).total_seconds())
                    else:
                        scraper_results[scraper_file] = {
                            "status": "❌ SYNTAX_ERROR",
                        ...
                        self.record_test_result("scrapers", f"{scraper_file}_syntax", "syntax_error",
                                              (datetime.datetime.now() - start_time).total_seconds(),
                                              error=result.stderr)
                else:
                    scraper_results[scraper_file] = {
                    ...
                    self.record_test_result("scrapers", f"{scraper_file}_syntax", "missing",
                                          (datetime.datetime.now() - start_time).total_seconds())
                ...
                self.record_test_result("scrapers", f"{scraper_file}_syntax", "error",
                                      (datetime.datetime.now() - start_time).total_seconds(),
                                      error=str(e))
        ...
        try:
            ...
            if ai_processor_path.exists():
                ...
                if result.returncode == 0:
                    ...
                    self.record_test_result("ai_workflow", "ai_processor_syntax", "pass",
                                          (datetime.datetime.now() - start_time).total_seconds())
                else:
                    ai_results["ai_processor"] = {
                        "status": "❌ SYNTAX_ERROR",
                    ...
                    self.record_test_result("ai_workflow", "ai_processor_syntax", "syntax_error",
                                          (datetime.datetime.now() - start_time).total_seconds(),
                                          error=result.stderr)
            else:
                ai_results["ai_processor"] = {
                ...
                self.record_test_result("ai_workflow", "ai_processor_syntax", "missing",
                                      (datetime.datetime.now() - start_time).total_seconds())
            ...
            self.record_test_result("ai_workflow", "ai_processor_syntax", "error",
                                  (datetime.datetime.now() - start_time).total_seconds(),
                                  error=str(e))
        ...
        try:
            ...
            if organizer_path.exists():
                ...
                if result.returncode == 0:
                    ...
                    self.record_test_result("ai_workflow", "article_organizer_syntax", "pass",
                                          (datetime.datetime.now() - start_time).total_seconds())
                else:
                    ai_results["article_organizer"] = {
                        "status": "❌ SYNTAX_ERROR", 
                    ...
                    self.record_test_result("ai_workflow", "article_organizer_syntax", "syntax_error",
                                          (datetime.datetime.now() - start_time).total_seconds(),
                                          error=result.stderr)
            else:
                ai_results["article_organizer"] = {
                ...
                self.record_test_result("ai_workflow", "article_organizer_syntax", "missing",
                                      (datetime.datetime.now() - start_time).total_seconds())
            ...
            self.record_test_result("ai_workflow", "article_organizer_syntax", "error",
                                  (datetime.datetime.now() - start_time).total_seconds(),
                                  error=str(e))
        ...
        for dir_name in critical_dirs:
            ...
            if dir_path.exists() and dir_path.is_dir():
                try:
                    ...
                    self.record_test_result("filesystem", f"directory_{dir_name}", "pass")
                except PermissionError:
                    fs_results[dir_name]["status"] = "❌ PERMISSION_ERROR"
                    self.record_test_result("filesystem", f"directory_{dir_name}", "permission_error")
            else:
                fs_results[dir_name]["status"] = "❌ MISSING"
                self.record_test_result("filesystem", f"directory_{dir_name}", "missing")
    ...
    def run_performance_snapshot(self) -> Dict:
        ...
        try:
            ...
            self.record_test_result("performance", "system_snapshot", "error", error=str(e))
    ...
    def generate_integration_report(self) -> str:
        ...
        cursor.execute('SELECT AVG(duration_seconds), MIN(duration_seconds), MAX(duration_seconds) FROM test_results WHERE duration_seconds > 0')
        ...
        self.results["summary"] = {
            ...
            "failed": status_counts.get("error", 0) + status_counts.get("syntax_error", 0),
            "skipped": status_counts.get("missing", 0) + status_counts.get("not_available", 0),
        ...
        report_path = self.test_dir / "reports" / "integration_tests" / f"integration_report_{self.timestamp}.json"
        ...
        md_report_path = self.test_dir / "reports" / "integration_tests" / f"integration_report_{self.timestamp}.md"
...
"""
        ...
        if "performance" in self.results:
            ...
            if "system" in perf:
                ...
                content += f"- **Memory Usage**: {sys_perf.get('memory_percent', 'N/A')}%\n"
                content += f"- **Disk Usage**: {sys_perf.get('disk_usage_percent', 'N/A')}%\n"
            
            if "python_process" in perf:
                proc_perf = perf["python_process"]
                content += f"- **Process Memory**: {proc_perf.get('memory_mb', 'N/A'):.1f} MB\n"
                content += f"- **Process Threads**: {proc_perf.get('num_threads', 'N/A')}\n"
